 ...
 *
 * The exit status is 0 on success, EXIT_NOTTY when the ioctl fails with
 * ENOTTY, which the kernel returns for a filesystem without a UUID, and
...
main(int argc, const char * const argv[])
{
	...
	if (ioctl(fd, FS_IOC_GETFSUUID, &fu) == -1)
		err(errno == ENOTTY ? EXIT_NOTTY : EXIT_FAILURE,
		    "FS_IOC_GETFSUUID failed");

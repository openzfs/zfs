...
#	2. Before expansion, copies=1 -> copies=2 of the same content must
#	   extend the DDT entry (two equal-ASIZE DVAs).
...
#	   BP; then scrub clean.
#	5. After expansion, verify a fresh same-epoch entry still extends.
#
...
typeset -r new_epoch_re='^index [[:xdigit:]]+ refcnt 2 phys 0 DVA\[0\]=<0:[[:xdigit:]]+:2c000> DVA\[1\]=<0:[[:xdigit:]]+:2c000> '
...
#
# RAIDZ expansion feature activation is permanent, but safe later extensions
# must continue to work. Prove a fresh entry can gain a copy in the new epoch.
#
log_must rm "$mnt/target-1" "$mnt/target-2"
sync_pool "$TESTPOOL"
drain_ddt_logs
log_must grep -q 'All DDTs are empty' "$ddt_dump"

log_must zfs set copies=1 "$TESTPOOL"
log_must dd if=/dev/urandom of="$mnt/new-epoch-1" \
    bs="$block_size" count="1"
sync_pool "$TESTPOOL"

log_must zfs set copies=2 "$TESTPOOL"
log_must dd if="$mnt/new-epoch-1" of="$mnt/new-epoch-2" \
    bs="$block_size" count="1"
sync_pool "$TESTPOOL"

drain_ddt_logs
assert_one_ddt_entry
log_must grep -Eq "$new_epoch_re" "$ddt_dump"
log_must cmp "$mnt/new-epoch-1" "$mnt/new-epoch-2"

log_pass "FDT extension preserves RAIDZ expansion geometry epochs"

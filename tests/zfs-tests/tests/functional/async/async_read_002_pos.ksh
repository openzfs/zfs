...
#
# DESCRIPTION:
#	Verify async DIO reads scale with iodepth. With true async I/O
#	(-EIOCBQUEUED), higher iodepth should yield higher IOPS because
#	multiple reads can be in-flight simultaneously.
#
# STRATEGY:
#	1. Enable zfs_async_dio_enabled
#	2. Run fio randread with iodepth=1,4,16,64 and log IOPS at each
#	3. Verify data integrity at iodepth=64
#	4. Check that iodepth=64 IOPS >= iodepth=1 IOPS
#
...
log_assert "Verify async DIO reads with libaio scale with iodepth"
...
for iodepth in 1 4 16 64; do
	log_note "Benchmarking libaio iodepth=$iodepth..."
	iops=$(async_dio_read_iops "$mntpnt" "libaio" $iodepth "$runtime" \
	    "scale-iod${iodepth}")
...
log_note "============================================"
log_note "IOPS Scaling (libaio randread, 128K, async enabled):"
...
log_pass "Async DIO reads with libaio scale with iodepth"

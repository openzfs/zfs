...
#
# DESCRIPTION:
#	Benchmark sync vs async DIO reads across 8 cells:
...
function cleanup
{
	log_must zfs set primarycache=all $TESTPOOL/$TESTFS
	rm -f "$mntpnt/async"*
}

log_assert "Compare sync vs async DIO reads: threads × iodepth matrix"
...
iops_s1d1=$(async_dio_iops "$mntpnt" "libaio" "read" 1 "$runtime" "sync-1j-1d")
log_note "Sync  1 job  x iodepth=1  → $iops_s1d1 IOPS"

iops_s1d64=$(async_dio_iops "$mntpnt" "libaio" "read" 64 "$runtime" "sync-1j-64d")
log_note "Sync  1 job  x iodepth=64 → $iops_s1d64 IOPS"

iops_sNj1=$(async_dio_iops "$mntpnt" "libaio" "read" 1 "$runtime" "sync-${NJ}j-1d" $NJ)
log_note "Sync ${NJ} jobs x iodepth=1  → $iops_sNj1 IOPS"

iops_sNj64=$(async_dio_iops "$mntpnt" "libaio" "read" 64 "$runtime" "sync-${NJ}j-64d" $NJ)
...
iops_a1d1=$(async_dio_iops "$mntpnt" "libaio" "read" 1 "$runtime" "async-1j-1d")
log_note "Async 1 job  x iodepth=1  → $iops_a1d1 IOPS"

iops_a1d64=$(async_dio_iops "$mntpnt" "libaio" "read" 64 "$runtime" "async-1j-64d")
log_note "Async 1 job  x iodepth=64 → $iops_a1d64 IOPS"

iops_aNj1=$(async_dio_iops "$mntpnt" "libaio" "read" 1 "$runtime" "async-${NJ}j-1d" $NJ)
log_note "Async ${NJ} jobs x iodepth=1  → $iops_aNj1 IOPS"

iops_aNj64=$(async_dio_iops "$mntpnt" "libaio" "read" 64 "$runtime" "async-${NJ}j-64d" $NJ)
log_note "Async ${NJ} jobs x iodepth=64 → $iops_aNj64 IOPS"

async_dio_verify "$mntpnt" "libaio" 64

log_note "============================================"
log_note " Results matrix (libaio randread, 128K, primarycache=metadata):"
log_note ""
...
log_pass "Async vs sync DIO reads: threads × iodepth matrix complete"

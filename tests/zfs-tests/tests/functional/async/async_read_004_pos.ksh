...
#
# DESCRIPTION:
#	Verify that a vectored asynchronous Direct I/O read is carried out by
#	the Direct I/O path and delivers the whole request.
#
# STRATEGY:
...
#	3. Read a page-aligned file with 2, 4 and 16 page-sized segments.  Two
#	   or more segments is what makes the kernel describe the request with
#	   a backing iovec array rather than the single pointer a one-segment
#	   request gets; 16 also puts it past UIO_FASTIOV.
...
#	   out either way depending on what has reused it since.
#	5. A single-segment read is the control.
#
...
# aio_dio_readv is built when the libaio-devel package is installed.
command -v aio_dio_readv > /dev/null ||
    log_unsupported "This test requires aio_dio_readv."

typeset mntpnt=$(get_prop mountpoint $TESTPOOL/$TESTFS)
typeset testfile="$mntpnt/async_readv"
...
log_assert "Vectored async Direct I/O reads are served by the Direct I/O path"
...
log_must aio_dio_readv "$testfile" 1 16

# Two or more segments carry a backing array that the submitting call owns.
for nseg in 2 4 16; do
	for pass in 1 2 3 4; do
		log_must aio_dio_readv "$testfile" $nseg 16
	done
done

log_pass "Vectored async Direct I/O reads are served by the Direct I/O path"

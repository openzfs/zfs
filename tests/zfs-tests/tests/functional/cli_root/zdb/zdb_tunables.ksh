...
log_must eval 'zdb -o info=zfs_recover | grep -qE "^zfs_recover \[\w+ r\w]: .+"'
...
log_must test $(zdb -o show | head -50 | grep -cE "^\w+: .+") -eq 50

# "info" by itself shows info about all tunables
# like previous test, we limit and then count
log_must test $(zdb -o info | head -50 | grep -cE "^\w+ \[\w+ r\w]: .+") -eq 50

...
#
# DESCRIPTION:
# Verify that 'zpool contract' with a non-existent vdev name
# produces an error and does not crash.
#
# STRATEGY:
# 1. Create an anymirror1 pool with 3 disks.
# 2. Attempt 'zpool contract' with a vdev name that does not exist.
...
log_assert "'zpool contract' with non-existent vdev name must fail"
log_onexit cleanup
...
#
# Use a vdev name that does not exist in the pool
#
log_note "DEBUG: testing 'zpool contract' with non-existent vdev name"
log_mustnot zpool contract $TESTPOOL bogus_vdev_name \
...
errmsg=$(zpool contract $TESTPOOL bogus_vdev_name \
...
log_pass "'zpool contract' with non-existent vdev name must fail"

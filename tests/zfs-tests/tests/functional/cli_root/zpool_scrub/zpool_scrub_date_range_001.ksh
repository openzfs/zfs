...
#     4. Repeat steps 1, 2, and 3 four more times.
#     5. Inject checksum errors into all 5 files.
...
#     9. Repeat steps 6–8 for each of the remaining 4 files.
#
...
function write_pool
{
	...
	log_must dd if=/dev/random of=$TESTDIR/${fname} bs=1M count=1
	log_must zpool export $TESTPOOL
	log_must zpool import $TESTPOOL
}
...
for i in `seq 0 4`; do
	log_must zpool scrub -w -S "${date_list[$i]}" -E "${date_list[$((i))]}" $TESTPOOL

...
#
# Copyright (c) 2019 by Klara Inc.  All rights reserved.
#
...
#
# DESCRIPTION:
# 'zpool prefetch ddt <pool>' can successfully prefetch a pool's DDT on demand.
#
# STRATEGY:
# 1. Build up storage pool with deduplicated dataset.
# 2. Export the pool.
# 3. Import the pool, and use `zpool prefetch ddt` to prefetch its table.
# 4. Verify the DDT was prefetched successfully
#

verify_runnable "both"

log_assert "'zpool prefetch ddt <pool>' can successfully prefetch the DDT for a pool."
...
log_must zpool prefetch ddt $TESTPOOL
...
log_must test "${reimport.cached}" -le "8192"
...
log_pass "'zpool prefetch ddt <pool>' success."

...
#
# DESCRIPTION:
# Verify that an anymirror1 vdev can be added to an existing
# traditional mirror pool.
#
# STRATEGY:
# 1. Create a pool with a traditional mirror vdev.
# 2. Add an anymirror1 vdev to the pool using -f.
...
log_assert "anymirror1 vdev can be added to a traditional mirror pool"
log_onexit cleanup

create_sparse_files "disk" 4 $MINVDEVSIZE2

log_must zpool create $TESTPOOL mirror $disk0 $disk1
log_must poolexists $TESTPOOL

log_must zpool add -f $TESTPOOL anymirror1 $disk2 $disk3
...
log_pass "anymirror1 vdev can be added to a traditional mirror pool"

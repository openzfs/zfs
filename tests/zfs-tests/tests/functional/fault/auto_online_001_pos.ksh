...
#
# Method: Creates a pool, exports pool, offlines disk,
# imports pool with missing disk, then onlines disk.
# ZED polls for an event change for online disk to be automatically
# added back to the pool.
# Creates a raidz1 zpool using persistent disk path names
# (ie not /dev/sdc). Tests import using pool guid and cache file.
#
...
pool=auto-online_test
fault_cachefile=$STF_SUITE/tests/functional/fault/zpool_fault.cache
target=$pool
...
for import_option in "-c $fault_cachefile -a" "$target"
do
	if is_loop_device $disk1; then
		#create a pool with one scsi_debug device and 3 loop devices
		log_must $ZPOOL create -f $pool raidz1 $SDDEVICE_ID $disk1 \
		    $disk2 $disk3
	elif ( is_real_device $disk1 || is_mpath_device $disk1 ); then
		log_must $ZPOOL create -f $pool raidz1 ${devs_id[0]} \
		    ${devs_id[1]} ${devs_id[2]}
	else
	...
	if [[ -e "$fault_cachefile" ]]; then
		log_must $RM $fault_cachefile
	fi
	log_must $CP /etc/zfs/zpool.cache $fault_cachefile
	#pool guid import
	...
	do
		...
		log_must $ZPOOL import $import_option
		check_state $pool "" "degraded"
		if (($? != 0)); then
			log_fail "$pool is not degraded"
		fi
		...
		check_state $pool "" "online"
		if (($? != 0)); then
			log_fail "$pool is not back online"
		fi
		$SLEEP 2
	done
	log_must $RM $fault_cachefile

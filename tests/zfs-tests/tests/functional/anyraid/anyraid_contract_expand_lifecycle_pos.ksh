...
#
# DESCRIPTION:
# Full lifecycle test: create pool, contract a disk, then attach a
# new disk and rebalance. Verifies data integrity throughout.
#
# STRATEGY:
# 1. Create anymirror1 pool with 5 disks
# 2. Write data and record checksums
# 3. Contract one disk (5 -> 4 disks), wait, verify checksums
# 4. Attach a new disk (4 -> 5 disks)
# 5. Rebalance, wait, verify checksums
...
log_assert "Contract then expand lifecycle preserves data integrity"
...
#
# Phase 1: Contract one disk (5 -> 4 disks).
#
log_note "DEBUG: phase 1 - contracting vdev_file.4 (5 -> 4 disks)"
...
[[ "$cap_contracted" -lt "$cap_original" ]] || \
...
#
# Phase 2: Attach a new disk and rebalance (4 -> 5 disks).
#
log_note "DEBUG: creating new disk for expansion"
log_must truncate -s 768M $TEST_BASE_DIR/vdev_file.5

log_note "DEBUG: phase 2 - attaching vdev_file.5 and rebalancing"
...
[[ "$cap_expanded" -gt "$cap_contracted" ]] || \
...
log_pass "Contract then expand lifecycle preserves data integrity"

...
#
# DESCRIPTION:
# Write various data patterns to an anyraidz1:2 pool, contract the pool
# by removing one disk, then verify all data reads back correctly.
# This tests raidz-style AnyRAID with parity=1 and data_width=2.
#
# STRATEGY:
# 1. Create pool with anyraidz1:2 (5 disks, min width=3, so 2 surplus)
...
log_assert "anyraidz1:2 contraction preserves data correctly across export/import"
...
# Verify small files.
log_note "DEBUG: verifying small files"
idx=0
while (( idx < 10 )); do
...
# Verify medium files.
log_note "DEBUG: verifying medium files"
idx=0
while (( idx < 5 )); do
...
# Verify large file.
log_note "DEBUG: verifying large file"
...
log_pass "anyraidz1:2 contraction preserves data correctly across export/import"

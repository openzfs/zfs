...
#
# DESCRIPTION:
# Contraction with mixed-size disks, removing the smallest disk.
# Tiles from the small disk are redistributed to larger disks.
#
# STRATEGY:
# 1. Create anymirror1 pool with 3 disks at 768M and 2 disks at 2G
# 2. Write data and record checksums
# 3. Contract by removing one of the 768M disks
# 4. Wait for relocation to complete
# 5. Verify checksums, capacity, pool health
#
...
log_must truncate -s 2G $TEST_BASE_DIR/vdev_file.{3,4}

log_note "DEBUG: setting tile size to 64MiB"
set_tunable64 ANYRAID_MIN_TILE_SIZE 67108864

log_assert "Contraction with mixed-size disks removing smallest preserves integrity"

log_note "DEBUG: creating pool with 3 small + 2 large disks"
...
#
# Contract the pool by removing the smallest disk (disk 0, 768M).
#
log_note "DEBUG: starting contraction to remove vdev_file.0 (768M)"
...
log_pass "Contraction with mixed-size disks removing smallest preserves integrity"

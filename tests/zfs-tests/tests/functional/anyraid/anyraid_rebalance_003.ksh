...
#
# DESCRIPTION:
# Anyraid rebalance with a same-size disk preserves data integrity
#
# STRATEGY:
# 1. Create an anymirror1 vdev with equal-sized disks
# 2. Write data and record checksums
# 3. Attach a new disk of the same size
# 4. Run rebalance
...
log_assert "Anyraid rebalance with a same-size disk preserves data integrity"

log_must create_pool $TESTPOOL anymirror1 $TEST_BASE_DIR/vdev_file.{0,1,2,3,4}

#
# Write several files and record their checksums before rebalance.
#
...
#
# Attach a same-size disk and rebalance. Since the new disk is the
# same size as the existing disks, the capacity increase should be
# modest (one more disk contributing tiles).
#
...
log_pass "Anyraid rebalance with a same-size disk preserves data integrity"

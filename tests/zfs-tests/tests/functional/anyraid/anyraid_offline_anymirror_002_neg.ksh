...
#
# DESCRIPTION:
# Offlining too many disks from an anymirror1 pool is rejected.
# An anymirror1 pool with 2 disks (minimum for parity=1) can
# tolerate 1 offline disk, but offlining the second must fail.
# This test is self-contained.
#
# STRATEGY:
# 1. Create an anymirror1 pool with 2 disks (minimum for parity=1).
# 2. Write data and record xxh128 checksums.
# 3. Offline one disk (succeeds, pool DEGRADED).
# 4. Attempt to offline the second disk (must fail).
...
log_assert "Offlining all disks from a minimum anymirror1 pool is rejected"

#
# Create sparse disk files and pool with minimum disks for parity=1.
#
log_must create_sparse_files "disk" 2 $DEVSIZE
log_must zpool create -f $TESTPOOL anymirror1 $disk0 $disk1
...
log_pass "Offlining all disks from a minimum anymirror1 pool is rejected"

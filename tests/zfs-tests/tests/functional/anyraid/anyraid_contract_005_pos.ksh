...
#
# DESCRIPTION:
# Contraction with mixed-size disks, removing the largest disk.
# Tiles from the large disk must fit on the remaining smaller disks.
#
# STRATEGY:
# 1. Create anymirror1 pool with 4 disks at 768M and 1 disk at 2G
# 2. Write a small amount of data and record checksums
# 3. Contract by removing the 2G disk
# 4. Wait for relocation to complete
# 5. Verify checksums, capacity decreased, pool health
#
...
log_note "DEBUG: creating mixed-size sparse files"
log_must truncate -s 768M $TEST_BASE_DIR/vdev_file.{0,1,2,3}
log_must truncate -s 2G $TEST_BASE_DIR/vdev_file.4

log_note "DEBUG: setting tile size to 64MiB"
set_tunable64 ANYRAID_MIN_TILE_SIZE 67108864

log_assert "Contraction with mixed-size disks removing largest preserves integrity"

log_note "DEBUG: creating pool with 4 small + 1 large disk"
log_must create_pool $TESTPOOL anymirror1 \
	$TEST_BASE_DIR/vdev_file.{0,1,2,3,4}

#
# Write a small amount of data and record checksums.
# Keep data small so tiles from the large disk can fit on remaining disks.
#
...
#
# Contract the pool by removing the largest disk (disk 4, 2G).
#
log_note "DEBUG: starting contraction to remove vdev_file.4 (2G)"
...
log_pass "Contraction with mixed-size disks removing largest preserves integrity"

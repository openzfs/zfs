...
#
# DESCRIPTION:
# Write various data patterns to an anyraidz3:3 AnyRAID pool, contract
# the pool by removing one disk, then verify all data reads back
# correctly. This tests raidz3 with data width of 3.
#
# STRATEGY:
# 1. Create pool with anyraidz3:3 (8 disks: 3 parity + 3 data = 6 min, use 8)
...
cleanup() {
	...
	rm -f $TEST_BASE_DIR/vdev_file.{0,1,2,3,4,5,6,7}
}

log_onexit cleanup

log_note "DEBUG: creating sparse files (8 disks)"
log_must truncate -s 1G $TEST_BASE_DIR/vdev_file.{0,1,2,3,4,5,6,7}

log_note "DEBUG: setting tile size to 64MiB"
set_tunable64 ANYRAID_MIN_TILE_SIZE 67108864

log_assert "anyraidz3:3 contraction preserves data correctly across export/import"

log_note "DEBUG: creating anyraidz3:3 pool with 8 disks"
log_must create_pool $TESTPOOL anyraidz3:3 \
	$TEST_BASE_DIR/vdev_file.{0,1,2,3,4,5,6,7}
...
#
# Contract the pool by removing disk 7.
#
...
log_note "DEBUG: starting contraction to remove vdev_file.7"
log_must zpool contract $TESTPOOL $vdev_name \
	$TEST_BASE_DIR/vdev_file.7
...
rm -f $TEST_BASE_DIR/vdev_file.7
...
log_pass "anyraidz3:3 contraction preserves data correctly across export/import"

...
#
# DESCRIPTION:
# Resilvering a disk during an AnyRAID rebalance is handled
# correctly. The rebalance pauses, and then resumes after the resilver
# completes. After the rebalance completes, the pool recovers to ONLINE
...
# 4. Resilver one of the original disks during rebalance.
# 5. Verify pool handles the offline gracefully.
# 6. Wait for rebalance and any resilver/scrub to complete.
...
cleanup() {
	...
	rm -f $TEST_BASE_DIR/vdev_file.{0,1,2,3,4,5,6}
}

log_onexit cleanup

log_assert "Disk resilver during anyraid rebalance is handled correctly"

#
# Create backing files.
#
log_must truncate -s 768M $TEST_BASE_DIR/vdev_file.{0,1,2,3,4,6}
...
log_must zpool replace $TESTPOOL $TEST_BASE_DIR/vdev_file.2 $TEST_BASE_DIR/vdev_file.6
...
#
# Clear any transient errors from the resilver.
#
...
if (( failedcount > 0 )); then
	log_fail "$failedcount of $file_count files had wrong checksums" \
		"after resilver during rebalance"
fi
...
log_pass "Disk resilver during anyraid rebalance is handled correctly"

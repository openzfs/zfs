...
log_note ddt_zap_default_bs = $ORIG_DDT_ZAP_DEFAULT_BS
...
function cleanup
{
	...
	log_must set_tunable32 TXG_TIMEOUT $ORIG_TXG_TIMEOUT
}
...
function do_setup
{
	# Create a general VDEV and special dedup VDEV
...
{
	zpool get -Hp -o value dedup_table_size $POOL
}

function dedup_table_quota
{
	zpool get -Hp -o value dedup_table_quota $POOL
}

function ddt_entries
{
	entries=$(zpool status -D $POOL | \
...
function ddt_dedup_vdev_limit
{
	...
	# add a dedicated dedup VDEV and enable an automatic quota
	log_must zpool add $POOL dedup $VDEV_DEDUP
	log_must zpool set dedup_table_quota=auto $POOL

	log_must zfs set recordsize=1K $POOL
	log_must zfs set compression=zstd $POOL

	# Generate a working set to fill up the dedup allocation class
	fio '--directory=/var/tmp/dedup_mount' '--name=dedup-filler-1' \

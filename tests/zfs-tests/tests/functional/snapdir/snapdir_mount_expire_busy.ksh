...
log_assert "Verify that busy snapshot automounts do not expire."
...
# Create a pool and a snapshot, with some files we can stat later
create_pool $TESTPOOL $DISKS
log_must zfs create -o snapdir=visible -o mountpoint=$TESTDIR/fs $TESTPOOL/$TESTFS
for s in {1..5} ; do
	log_must touch $TESTDIR/fs/$s
done
...
# Keep the mount busy for a few seconds, checking its mounted each time
for s in {1..5} ; do
	log_must test -f $TESTDIR/fs/$SNAPROOT/snap/$s
	sleep 1
	log_must test "$(get_mount_paths $TESTPOOL/$TESTFS@snap)" == "$TESTDIR/fs/$SNAPROOT/snap"
done
...
log_pass "Busy snapshot automounts do not expire."

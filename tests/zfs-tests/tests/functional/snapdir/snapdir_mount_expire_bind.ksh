...
log_assert "Verify binding then expiring snapshot automount allows another automount"

if ! is_linux ; then
	log_unsupported "EXPIRE_SNAPSHOT tunable and mount --bind only available on Linux"
fi
...
# Wait for the expiry, then check the original has been unmounted but the bind remains
...
log_pass "Snapshot automount succeeds after previous automount is bound then expired."

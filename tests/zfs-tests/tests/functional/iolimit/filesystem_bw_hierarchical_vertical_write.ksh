...
for lvl0 in none 1M 3M 5M; do
	for lvl1 in none 1M 3M 5M; do
		for lvl2 in none 1M 3M 5M; do
			...
			log_must zfs set iolimit_bw_write=$lvl0 "$TESTPOOL/$TESTFS/lvl0"
			log_must zfs set iolimit_bw_write=$lvl1 "$TESTPOOL/$TESTFS/lvl0/lvl1"
			log_must zfs set iolimit_bw_write=$lvl2 "$TESTPOOL/$TESTFS/lvl0/lvl1/lvl2"

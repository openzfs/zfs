...
for lvl0 in none 1M 3M 5M; do
	for lvl1 in none 1M 3M 5M; do
		for lvl2 in none 1M 3M 5M; do
			...
			log_must zfs set iolimit_bw_read=$lvl0 "$TESTPOOL/$TESTFS/lvl0"
			log_must zfs set iolimit_bw_read=$lvl1 "$TESTPOOL/$TESTFS/lvl0/lvl1"
			log_must zfs set iolimit_bw_read=$lvl2 "$TESTPOOL/$TESTFS/lvl0/lvl1/lvl2"
			log_must iolimit_bw_read 5 5 "/dev/zvol/$TESTPOOL/$TESTFS/lvl0/lvl1/lvl2"
		done
	done
done

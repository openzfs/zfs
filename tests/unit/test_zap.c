...
mock_zap_create_norm_microzap(int normflags) {
	/*
	 * We use DMU_OTN_ZAP_DATA (an OTN type) so that DMU_OT_BYTESWAP()
	 * extracts DMU_BSWAP_ZAP directly from the encoded bits without
	 * consulting dmu_ot[].  MicroZAPs must not promote to TinyZAP, and
	 * DMU_OTN_ZAP_DATA != DMU_OT_DIRECTORY_CONTENTS enforces that.
	 */
...
static dnode_t *
mock_zap_create_norm_tinyzap(int normflags) {
	/*
	 ...
	 */
	mock_dnode_t *mdn = mock_dnode_create(4096, DMU_OT_DIRECTORY_CONTENTS);
	dnode_t *dn = (dnode_t *)mdn;
	dmu_tx_t *tx = (dmu_tx_t *)mock_tx_create();
	mzap_create_impl(dn, normflags, 0, tx);
...
#define	mock_zap_create_tinyzap()	mock_zap_create_norm_tinyzap(0)
...
mock_zap_create_norm_params(const MunitParameter params[], const char *key,
	...
	if (type == NULL)
		...
		return (mock_zap_create_norm_tinyzap(normflags));
...
test_tinyzap_cursor_multi_int(const MunitParameter params[], void *data)
{
	...
	mock_dnode_t *mdn = mock_dnode_create(512, DMU_OT_DIRECTORY_CONTENTS);
	dnode_t *dn = (dnode_t *)mdn;
	dmu_tx_t *tx = (dmu_tx_t *)mock_tx_create();
	mzap_create_impl(dn, 0, 0, tx);

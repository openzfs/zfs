...
zed_event_seek(struct zed_conf *zcp, uint64_t saved_eid, int64_t saved_etime[])
{
	...
	while ((eid < saved_eid) && !found) {
		/* FIXME: Change do_block to flags w/ ZEVENT_NONBLOCK. */
		rv = zpool_events_next(zcp->zfs_hdl, &nvl, &n_dropped,
		    do_block, zcp->zevent_fd);
...
_zed_event_add_nvpair(uint64_t eid, zed_strings_t *zsp, nvpair_t *nvp)
{
	...
	n = strlcpy(buf, ZED_ENV_VAR_PREFIX, sizeof (buf));
	if (n >= sizeof (buf)) {
		zed_log_msg(LOG_WARNING,
...
_zed_event_add_env_restrict(uint64_t eid, zed_strings_t *zsp)
{
	...
	for (pp = env_restrict; *pp; pp++) {
		if (zed_strings_add(zsp, *pp) < 0) {
			p = strchr(*pp, '=');
			len = (p != NULL) ? p - *pp : strlen(*pp);
			zed_log_msg(LOG_WARNING,
			    "Failed to add env var %.*s for eid=%llu: %s",
			    len, *pp, eid, strerror(ENOMEM));
			continue;
		}
	}
}
...
_zed_event_add_env_preserve(uint64_t eid, zed_strings_t *zsp)
{
	...
	for (pp = env_preserve; *pp; pp++) {
		...
		n = snprintf(buf, sizeof (buf), "%s=%s", *pp, p);
		if ((n < 0) || (n >= sizeof (buf))) {
			zed_log_msg(LOG_WARNING,
			    "Failed to add env var %s for eid=%llu: %s",
			    *pp, eid, "Exceeded buffer size");
			continue;
		}
		if (zed_strings_add(zsp, buf) < 0) {
			zed_log_msg(LOG_WARNING,
			    "Failed to add env var %s for eid=%llu: %s",
			    *pp, eid, strerror(ENOMEM));
			continue;
		}
	}
}
...
_zed_event_add_time_strings(uint64_t eid, zed_strings_t *zsp, int64_t etime[])
{
	...
	n = snprintf(buf, sizeof (buf), "%s%s=%lld",
	    ZED_ENV_VAR_PREFIX, "TIME_SECS", (long long int) etime[0]);
	if ((n < 0) || (n >= sizeof (buf))) {
		zed_log_msg(LOG_WARNING, "Failed to add env var TIME_SECS "
		    "for eid=%llu: %s", eid, "Exceeded buffer size");
	} else if (zed_strings_add(zsp, buf) < 0) {
		zed_log_msg(LOG_WARNING, "Failed to add env var TIME_SECS "
		    "for eid=%llu: %s", eid, strerror(ENOMEM));
	}

	n = snprintf(buf, sizeof (buf), "%s%s=%lld",
	    ZED_ENV_VAR_PREFIX, "TIME_NSECS", (long long int) etime[1]);
	if ((n < 0) || (n >= sizeof (buf))) {
		zed_log_msg(LOG_WARNING, "Failed to add env var TIME_NSECS "
		    "for eid=%llu: %s", eid, "Exceeded buffer size");
	} else if (zed_strings_add(zsp, buf) < 0) {
		zed_log_msg(LOG_WARNING, "Failed to add env var TIME_NSECS "
		    "for eid=%llu: %s", eid, strerror(ENOMEM));
	...
	n = snprintf(buf, sizeof (buf), "%s%s=",
	    ZED_ENV_VAR_PREFIX, "TIME_STRING");
	if ((n < 0) || (n >= sizeof (buf))) {
		zed_log_msg(LOG_WARNING, "Failed to add env var TIME_STRING "
		    "for eid=%llu: %s", eid, strerror(ENOMEM));
	} else if (!(stp = localtime((const time_t *) &etime[0]))) {
		zed_log_msg(LOG_WARNING, "Failed to add env var TIME_STRING "
		    "for eid=%llu: %s", eid, "localtime error");
	} else if (!strftime(buf + n, sizeof (buf) - n,
	    "%Y-%m-%d %H:%M:%S%z", stp)) {
		zed_log_msg(LOG_WARNING, "Failed to add env var TIME_STRING "
		    "for eid=%llu: %s", eid, "strftime error");
	} else if (zed_strings_add(zsp, buf) < 0) {
		zed_log_msg(LOG_WARNING, "Failed to add env var TIME_STRING "
		    "for eid=%llu: %s", eid, strerror(ENOMEM));
	}
...
/*
 * Add the zevent [subclass] string.
 */
static void
_zed_event_add_subclass(uint64_t eid, zed_strings_t *zsp, const char *subclass)
{
	int n;
	char buf[1024];
	...
	n = snprintf(buf, sizeof (buf), "%s%s=%s",
	    ZED_ENV_VAR_PREFIX, "SUBCLASS", subclass);
	if ((n < 0) || (n >= sizeof (buf))) {
		zed_log_msg(LOG_WARNING,
		    "Failed to add env var SUBCLASS for eid=%llu: %s",
		    eid, "Exceeded buffer size");
	} else if (zed_strings_add(zsp, buf) < 0) {
		zed_log_msg(LOG_WARNING,
		    "Failed to add env var SUBCLASS for eid=%llu: %s",
		    eid, strerror(ENOMEM));
...
zed_event_service(struct zed_conf *zcp)
{
	...
	/* FIXME: Change do_block to flags w/o ZEVENT_NONBLOCK. */
	rv = zpool_events_next(zcp->zfs_hdl, &nvl, &n_dropped,
	    do_block, zcp->zevent_fd);
	...
	if (nvlist_lookup_uint64(nvl, "eid", &eid) != 0) {
		...
		_zed_event_add_subclass(eid, zsp,

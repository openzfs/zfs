...
dump_object(objset_t *os, uint64_t object, int verbosity, int *print_header)
{
	...
	void *bonus = NULL;
	size_t bsize = 0;
	char iblk[32], dblk[32], lsize[32], asize[32], fill[32], szsec[32];
	char bonus_size[32];
	char aux[50];
	int error;

	if (*print_header) {
		(void) printf("\n%10s  %3s  %5s  %5s  %5s  %5s  %5s  %6s  %s\n",
		    "Object", "lvl", "iblk", "dblk", "dsize", "szsec",
		    "lsize", "%full", "type");
		*print_header = 0;
	}
	...
	zdb_nicenum(doi.doi_szsec, szsec);
	...
	(void) printf("%10lld  %3u  %5s  %5s  %5s  %5s  %5s  %6s  %s%s\n",
	    (u_longlong_t)object, doi.doi_indirection, iblk, dblk,
	    asize, szsec, lsize, fill, ZDB_OT_NAME(doi.doi_type), aux);

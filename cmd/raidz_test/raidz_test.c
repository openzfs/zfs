...
init_raidz_golden_map(raidz_test_opts_t *opts, const int parity)
{
	...
	opts->zio_golden->io_data = abd_alloc_scatter(opts->rto_dsize);
	zio_test->io_data = abd_alloc_scatter(opts->rto_dsize);

	init_zio_data(opts->zio_golden);
	init_zio_data(zio_test);

	mprotect(opts->zio_golden->io_data, zio_test->io_size, PROT_READ);
...
run_gen_check(raidz_test_opts_t *opts)
{
	...
	IPRINT(DBLSEP);
	IPRINT("Testing parity generation...\n");

	for (impl_name = (char **)raidz_impl_names+1; *impl_name != NULL;
	    impl_name++) {

		IPRINT(SEP);
		IPRINT("\tTesting [%s] implementation...", *impl_name);
		...
		for (fn = 0; fn < RAIDZ_GEN_NUM; fn++) {
			...
			IPRINT("\t\tTesting method [%s] ...",
			    raidz_gen_name[fn]);

			if (!opts->rto_sanity)
				vdev_raidz_generate_parity(rm_test);

			if (0 == cmp_code(opts, rm_test, fn+1)) {
				IPRINT("[PASS]\n");
			} else {
				IPRINT("[FAIL]\n");
				err++;
...
unsigned
__run_rec_check(raidz_test_opts_t *opts, raidz_map_t *rm, const int fn)
{
	int x0, x1, x2;
	int tgtidx[3];
	int err = 0;
...
run_rec_check(raidz_test_opts_t *opts)
{
	...
	IPRINT(DBLSEP);
	IPRINT("Testing data reconstruction...\n");
	...
	for (impl_name = (char **)raidz_impl_names+1; *impl_name != NULL;
		...
		for (fn = 0; fn < RAIDZ_REC_NUM; fn++) {
			...
			if (0 == __run_rec_check(opts, rm_test, fn)) {
				IPRINT("[PASS]\n");
			} else {
				err++;
				IPRINT("[FAIL]\n");
...
run_sweep(void)
{
	...
	static const size_t size_v[] = { 1<<9, 21 * (1<<9), 13 * (1<<12), 1<<17,
	    (1<<20) - (1<<12), SPA_MAXBLOCKSIZE };

	size_t total_comb = ARR_LEN(size_v) * ARR_LEN(ashift_v) *
	    ARR_LEN(dcols_v) * offset_cnt;
	size_t tried_comb = 0;
	hrtime_t time_diff, start_time = gethrtime();
	raidz_test_opts_t *opts;
	int a, d, o, s;

	max_free_slots = free_slots = MAX(2, sysconf(_SC_NPROCESSORS_ONLN));

	mutex_init(&sem_mtx, NULL, MUTEX_DEFAULT, NULL);
	cv_init(&sem_cv, NULL, CV_DEFAULT, NULL);

	for (s = 0; s < ARR_LEN(size_v); s++)
	for (a = 0; a < ARR_LEN(ashift_v); a++)
	for (o = 0; o < offset_cnt; o++)
	for (d = 0; d < ARR_LEN(dcols_v); d++) {
		...
		if (++tried_comb % 20 == 0)
			PRINT("%lu/%lu... ", (ulong_t)tried_comb,
			    (ulong_t)total_comb);
		...
		while (cv_timedwait_sig(&sem_cv, &sem_mtx,
			...
			time_diff = (gethrtime() - start_time) / 1000000000ULL;
		...
		VERIFY3P(thread_create(NULL, 0,
		    (thread_func_t) sweep_thread, (void *) opts,
		    0, TS_RUN, 0, PTHREAD_CREATE_JOINABLE),
		    !=, NULL);
	}
exit:
	...
	if (sweep_state == SWEEP_ERROR) {
		PRINT("Sweep test failed! Failed option: \n");
		print_opts(&failed_opts, B_TRUE);
	} else {
		if (sweep_state == SWEEP_TIMEOUT)
			PRINT("Test timeout (%lus). Stopping...\n",
			    (ulong_t)rto_opts.rto_sweep_timeout);

		PRINT("Sweep test succeeded on %lu raidz maps!\n",
		    (ulong_t)tried_comb);
	}

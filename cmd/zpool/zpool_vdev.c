...
boolean_t
vdev_bdev_database_check(char *path, int *sector_size)
{
	...
	i = ioctl(fd, SG_IO, (unsigned long) &io_hdr);
	...
	for (i = 0; i < vdev_bdev_database_size; i++) {
		if (memcmp(inq_buff + 8, vdev_bdev_database[i].id, 24))
			continue;
...
make_leaf_vdev(nvlist_t *props, const char *arg, uint64_t is_log)
{
	...
	if (props != NULL) {
		...
		if (ashift == 0) {
			int sector_size;
			if (vdev_bdev_database_check(path, &sector_size) == B_TRUE)

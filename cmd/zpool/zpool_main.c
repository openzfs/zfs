...
get_usage(zpool_help_t idx)
{
	...
	case HELP_CONDENSE:
		return (gettext("\tcondense [-t <target>] [-c | -w] "
		    "[-a | <pool> ...]\n"));
	case HELP_VERSION:
...
condense_cb(zpool_handle_t *zhp, void *data)
{
	...
	int err = 0;
	for (pool_condense_type_t ty = 0; ty < POOL_CONDENSE_TYPES; ty++) {
		err = zpool_condense(zhp, cb->func, ty);
		if (err != 0)
			break;
	}
...
/*
 * zpool condense [-t <target>] [-c | -w] [-a | <pool> ...]
 *

...
extern uint_t reference_tracking_enable;

static importargs_t g_importargs;
static char *g_pool;
static boolean_t g_readonly;
static enum ddt_class zhack_scn_ddt_class_max = DDT_CLASS_DUPLICATE;
...
zhack_do_scrub(int argc, char **argv)
{
	...
	if (do_ddt_reset) {
		...
		if (scn->scn_phys.scn_ddt_class_max < zhack_scn_ddt_class_max) {
			if (verbose) {
				(void) fprintf(stderr,
				    "Forcibly resetting DDT scan class\n");
			}
			scn->scn_phys.scn_ddt_class_max =
			    zhack_scn_ddt_class_max;
	...
	do {
		...
		(void) fprintf(stderr,
		    ...
		    scn->scn_queues_pending,
		    (float)scn->scn_queues_pending * 100 / scnp->scn_to_examine,
		    issued,
...
main(int argc, char **argv)
{
	...
	if (strcmp(subcommand, "feature") == 0) {
		...
		return (zhack_do_scrub(argc, argv));
	} else {
		(void) fprintf(stderr, "error: unknown subcommand: %s\n",
		    subcommand);
		usage();

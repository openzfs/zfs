...
#define	NVP(buf, size, len, elem, type, vtype, ptype, format) { \
	vtype	value; \
	int rc; \
\
...
#define	NVPA(buf, size, len, elem, type, vtype, ptype, format) { \
	uint_t	i, count; \
	vtype	*value;  \
	int rc; \
\
 ...
 */
static int
nvlist_snprintf_impl(char *buf, size_t size, nvlist_t *list, int indent)
{
	nvpair_t	*elem = NULL;
	...
	while ((elem = nvlist_next_nvpair(list, elem)) != NULL) {
		...
		case DATA_TYPE_BYTE:
			NVP(buf, size, len, elem, byte, uchar_t, int, "%u");
			break;

		case DATA_TYPE_INT8:
			NVP(buf, size, len, elem, int8, int8_t, int, "%d");
			break;

		case DATA_TYPE_UINT8:
			NVP(buf, size, len, elem, uint8, uint8_t, int, "%u");
			break;

		case DATA_TYPE_INT16:
			NVP(buf, size, len, elem, int16, int16_t, int, "%d");
			break;

		case DATA_TYPE_UINT16:
			NVP(buf, size, len, elem, uint16, uint16_t, int, "%u");
			break;

		case DATA_TYPE_INT32:
			NVP(buf, size, len, elem, int32, int32_t, long, "%ld");
			break;

		case DATA_TYPE_UINT32:
			NVP(buf, size, len, elem, uint32, uint32_t, ulong_t,
			    "%lu");
			break;

		case DATA_TYPE_INT64:
			NVP(buf, size, len, elem, int64, int64_t, longlong_t,
			    "%lld");
			break;

		case DATA_TYPE_UINT64:
			NVP(buf, size, len, elem, uint64, uint64_t,
			    u_longlong_t, "%llu");
			break;

		case DATA_TYPE_STRING:
			NVP(buf, size, len, elem, string, const char *,
			    const char *, "'%s'");
			break;

		case DATA_TYPE_BYTE_ARRAY:
			NVPA(buf, size, len, elem, byte_array, uchar_t, int,
			    "%u");
			break;

		case DATA_TYPE_INT8_ARRAY:
			NVPA(buf, size, len, elem, int8_array, int8_t, int,
			    "%d");
			break;

		case DATA_TYPE_UINT8_ARRAY:
			NVPA(buf, size, len, elem, uint8_array, uint8_t, int,
			    "%u");
			break;

		case DATA_TYPE_INT16_ARRAY:
			NVPA(buf, size, len, elem, int16_array, int16_t, int,
			    "%d");
			break;

		case DATA_TYPE_UINT16_ARRAY:
			NVPA(buf, size, len, elem, uint16_array, uint16_t, int,
			    "%u");
			break;

		case DATA_TYPE_INT32_ARRAY:
			NVPA(buf, size, len, elem, int32_array, int32_t, long,
			    "%ld");
			break;

		case DATA_TYPE_UINT32_ARRAY:
			NVPA(buf, size, len, elem, uint32_array, uint32_t,
			    ulong_t, "%lu");
			break;

		case DATA_TYPE_INT64_ARRAY:
			NVPA(buf, size, len, elem, int64_array, int64_t,
			    longlong_t, "%lld");
			break;

		case DATA_TYPE_UINT64_ARRAY:
			NVPA(buf, size, len, elem, uint64_array, uint64_t,
			    u_longlong_t, "%llu");
			break;

		case DATA_TYPE_STRING_ARRAY:
			NVPA(buf, size, len, elem, string_array, const char *,
			    const char *, "'%s'");
			break;
		...
		case DATA_TYPE_NVLIST:
			...
			rc = nvlist_snprintf_impl(buf, size, nvlist_value,
			    indent + 4);
			if (rc < 0)
				return (rc);
			size = MAX((int)size - rc, 0);
		...
		case DATA_TYPE_NVLIST_ARRAY:
			...
			for (i = 0; i < count; i++) {
				...
				rc = nvlist_snprintf_impl(buf, size,
...
nvlist_snprintf(char *buf, size_t size, nvlist_t *list, int indent)
{
	...
	rc = nvlist_snprintf_impl(buf, size, list, indent);
	if (rc < 0)
		return (rc);

	return (rc + 1); /* Account for final null terminator */
}

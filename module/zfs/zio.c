...
zio_write_gang_block(zio_t *pio, metaslab_class_t *mc)
{
	...
	error = metaslab_alloc(spa, mc, SPA_GANGBLOCKSIZE,
	    ...
	    &pio->io_alloc_list, pio->io_allocator);
	if (error) {
		pio->io_error = error;
		return (pio);
	}
	...
	for (int g = 0; resid != 0; resid -= psize, g++) {
		...
		/*
		 * We do not reserve for the children writes, since we already
		 * reserved for the parent.  Unreserve though will be called
		 * for individual children.  We can do this since we reserve
		 * physical size, not allocated size, which might be bigger.
		 */
...
again:
	...
	error = metaslab_alloc(spa, mc, zio->io_size, bp,
	    ...
	    &zio->io_alloc_list, zio->io_allocator);
    ...
    uint64_t size, boolean_t *slog)
{
	...
	error = metaslab_alloc(spa, spa_log_class(spa), size, new_bp, 1,
	    txg, NULL, flags, &io_alloc_list, allocator);
	*slog = (error == 0);
	if (error != 0) {
...
zio_dva_throttle_done(zio_t *zio)
{
	...
	metaslab_group_alloc_decrement(zio->io_spa, vd->vdev_id,
	    pio->io_allocator, flags, pio->io_size);

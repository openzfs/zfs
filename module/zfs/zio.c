...
zio_dva_throttle_done(zio_t *zio)
{
	...
	/*
	 * The original allocation was against the local write. We are the vdev
	 * child IO for the top vdev; our parent is the vdev child IO for the
	 * parent vdev. So, we have to walk two steps up the tree.
	 */
	...
	metaslab_group_alloc_decrement(zio->io_spa, zio->io_vd->vdev_id,
	    pio->io_allocator, METASLAB_ASYNC_ALLOC, pio->io_size, tag);
...
zio_logical_io_done(zio_t *zio)
{
	...
	/* Inject data faults (targeting object type, bookmark or DVA) */
	...
	if (zio->io_error && !(zio->io_flags & (ZIO_FLAG_DONT_RETRY |
	    ZIO_FLAG_IO_RETRY | ZIO_FLAG_DIO_CHKSUM_ERR))) {

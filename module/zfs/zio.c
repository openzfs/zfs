...
zio_ddt_collision(zio_t *zio, ddt_t *ddt, ddt_entry_t *dde)
{
	...
	for (p = DDT_PHYS_SINGLE; p <= DDT_PHYS_TRIPLE; p++) {
		...
		if (lio != NULL && do_raw) {
			return (lio->io_size != zio->io_size ||
			    abd_cmp(zio->io_data, lio->io_data,
			    zio->io_size) != 0);
		} else if (lio != NULL) {
	...
	for (p = DDT_PHYS_SINGLE; p <= DDT_PHYS_TRIPLE; p++) {
		...
		if (ddp->ddp_phys_birth != 0 && do_raw) {
			...
			tmpbuf = abd_alloc_scatter(psize);
			...
			if (error == 0) {
				if (abd_cmp(tmpbuf, zio->io_data, psize) != 0)
					error = SET_ERROR(ENOENT);
			}

			abd_free(tmpbuf, psize);
			ddt_enter(ddt);
			return (error != 0);
			...
			if (error == 0) {
				if (abd_cmp(abuf->b_data, zio->io_orig_data,
				    zio->io_orig_size) != 0)
					error = SET_ERROR(ENOENT);

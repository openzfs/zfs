...
vdev_anyraid_load(vdev_t *vd)
{
	...
	if (va->vd_relocate.var_state == DSS_NONE ||
...
vdev_anyraid_io_start(zio_t *zio)
{
	...
	if (tile == NULL) {
		...
		for (int i = 0; i < width; i++) {
			...
			ASSERT(vans[i] &&
			    anyraid_freelist_alloc(&vans[i]->van_freelist) <
			    vans[i]->van_capacity);
    ...
    zfs_range_seg64_t *physical_rs, zfs_range_seg64_t *remain_rs)
{
	...
	switch (va->vd_parity_type) {
		case VAP_MIRROR:
		{
			...
			child_offset +=
			    VDEV_ANYRAID_START_OFFSET(cvd->vdev_ashift);
    ...
    uint64_t *good_writes, int flags, vdev_config_sync_status_t status)
{
	...
	for (uint64_t i = 0; i < anyraidvd->vdev_children; i++) {
		if (anyraidvd->vdev_child[i] == vd)
			disk_id = i;
		ASSERT(0 < va->vd_children[i]->van_capacity &&
		    va->vd_children[i]->van_capacity <= UINT16_MAX);
    ...
    struct rebal_node *receiver, uint32_t *tid)
{
	...
	for (int i = 0; i < dvan->van_freelist.af_next_off; i++) {
		...
		receiver->arr[task->vart_dest_idx] = donor->arr[i];
		donor->arr[i] = -1LL;
		return (B_TRUE);
	}
	return (B_FALSE);

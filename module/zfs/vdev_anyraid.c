...
anyraid_calculate_size(vdev_t *vd)
{
	...
	uint64_t disk_shift = vdev_anyraid_disk_shift;
	uint64_t min_size = vdev_anyraid_min_tile_size;
	uint_t toosmall = 0;
	...
	if (smallest_disk_size < 1 << disk_shift ||
		...
		ASSERT(toosmall);
		char **vdevs = kmem_alloc(toosmall * sizeof (*vdevs),
		    KM_SLEEP);
		int idx = 0;
		uint_t ashift = 0;
		for (int c = 0; c < vd->vdev_children; c++) {
		...
		if (spa->spa_create_info == NULL) {
			...
			fnvlist_add_string_array(nv, ZPOOL_CREATE_INFO_VDEV,
			    (const char *const *)vdevs, toosmall);

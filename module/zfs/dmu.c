...
#define	dmu_uiomove(data, sz, dir, uio)			\
	vn_io_fault_uiomove((data), (sz), GET_UIO_STRUCT((uio)))
#else
#define	dmu_uiomove(data, sz, dir, uio)			\
	zfs_uiomove((data), (sz), (dir), (uio))
#endif
...
dmu_buf_do_uio(dmu_buf_set_t *dbs, dmu_buf_t *db, uint64_t off,
    uint64_t sz, zfs_uio_rw_t dir)
{
	int err;
	zfs_uio_t *uio = (zfs_uio_t *)dbs->dbs_dc->dc_data_buf;
	uint64_t adv = zfs_uio_resid(uio);

	err = dmu_uiomove((char *)db->db_data + off, sz, dir, uio);
	if (err)
		dmu_buf_set_set_error(dbs, err);
	adv -= zfs_uio_resid(uio);
...
dmu_ctx_rele(dmu_ctx_t *dmu_ctx)
{
	...
	if (dmu_ctx->dc_complete_cb != NULL)
		rc = dmu_ctx->dc_complete_cb(dmu_ctx);
	return (rc);
}
...
#ifdef ZFS_DEBUG
	if (dmu_ctx->dc_flags & DMU_CTX_FLAG_UIO) {
		zfs_uio_t *uio = (zfs_uio_t *)data_buf;
		/* Make sure UIO callers pass in the correct offset. */
		ASSERT(zfs_uio_offset(uio) == offset);
	}
...
{
	return (dmu_read_impl(dn, NULL, 0, zfs_uio_offset(uio), size, uio,
...
dmu_read_uio(objset_t *os, uint64_t object, zfs_uio_t *uio, uint64_t size)
{
	...
	return (dmu_read_impl(NULL, os, object, zfs_uio_offset(uio), size, uio,
	    DMU_CTX_FLAG_UIO));
}
...
dmu_write_uio_dnode(dnode_t *dn, zfs_uio_t *uio, uint64_t size, dmu_tx_t *tx)
{
	...
	return (dmu_write_impl(dn, NULL, 0, zfs_uio_offset(uio), size, uio, tx,
    ...
    dmu_tx_t *tx)
{
	...
	DB_DNODE_ENTER(db);
	dn = DB_DNODE(db);
	err = dmu_write_impl(dn, NULL, 0, zfs_uio_offset(uio), size, uio, tx,
	    DMU_CTX_FLAG_UIO|DMU_CTX_FLAG_NO_HOLD);
	DB_DNODE_EXIT(db);
	return (err);
}
    ...
    dmu_tx_t *tx)
{
	...
	if (size == 0)
		return (0);
	return (dmu_write_impl(NULL, os, object, zfs_uio_offset(uio), size,

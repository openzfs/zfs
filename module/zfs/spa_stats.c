...
static void *
spa_stats_json_addr(kstat_t *ksp, loff_t n)
{
	if (n == 0)
...
static int
spa_stats_json_data(char *buf, size_t size, void *data)
{
	spa_t *spa = (spa_t *)data;

	return (spa_stats_json_generate(spa, buf, size));
}

static void
spa_stats_json_init(spa_t *spa)
{
	char *name;
	kstat_t *ksp;

	mutex_init(&spa->spa_stats_json.ssj_lock, NULL, MUTEX_DEFAULT, NULL);
	name = kmem_asprintf("zfs/%s", spa_name(spa));
	ksp = kstat_create(name, 0, "status_json", "misc", KSTAT_TYPE_RAW, 0,
	    KSTAT_FLAG_VIRTUAL | KSTAT_FLAG_NO_HEADERS);
	spa->spa_stats_json.ssj_kstat = ksp;
	if (ksp) {
		ksp->ks_lock = &spa->spa_stats_json.ssj_lock;
		ksp->ks_data = NULL;
		ksp->ks_private = spa;
		kstat_set_raw_ops(ksp, NULL, spa_stats_json_data,
		    spa_stats_json_addr);
		kstat_install(ksp);
	}
...
static void
spa_stats_json_destroy(spa_t *spa)
{
	if (spa->spa_stats_json.ssj_kstat)
		kstat_delete(spa->spa_stats_json.ssj_kstat);
	mutex_destroy(&spa->spa_stats_json.ssj_lock);
}
...
spa_stats_init(spa_t *spa)
{
	...
	spa_stats_json_init(spa);
}

void
spa_stats_destroy(spa_t *spa)
{
	spa_stats_json_destroy(spa);

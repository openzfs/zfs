...
#define	DDT_LOG_BS	(12)	/* 4K */
#define	DDT_LOG_IBS	(14)	/* 16K */

#define	DDT_LOG_BSIZE	(1ULL << DDT_LOG_BS)
...
#define	DDT_LOG_ENTRY_FLAT_SIZE	\
	(sizeof (ddt_log_entry_t) + sizeof (ddt_phys_flat_t))
#define	DDT_LOG_ENTRY_TRAD_SIZE	\
	(sizeof (ddt_log_entry_t) + sizeof (ddt_phys_trad_t))
...
ddt_log_update_header(ddt_t *ddt, ddt_log_t *ddl, dmu_tx_t *tx)
{
	...
	hdr->dlh_nblocks = ddl->ddl_nblocks;
...
ddt_log_create_one(ddt_t *ddt, ddt_log_t *ddl, uint_t n, dmu_tx_t *tx)
{
	...
	ddl->ddl_object = dmu_object_alloc_ibs(ddt->ddt_os,
	    DMU_OTN_UINT64_METADATA, DDT_LOG_BSIZE, DDT_LOG_IBS,
...
ddt_log_destroy_one(ddt_t *ddt, ddt_log_t *ddl, uint_t n, dmu_tx_t *tx)
{
	...
	ASSERT0(ddl->ddl_nblocks);

	char name[DDT_NAMELEN];
	ddt_log_name(ddt, name, n);

	ASSERT0(zap_contains(ddt->ddt_os, ddt->ddt_dir_object, name));
...
ddt_log_update_stats(ddt_t *ddt)
{
	...
	ddo->ddo_mspace = ddo->ddo_count * sizeof (ddt_log_entry_t);
...
ddt_log_begin(ddt_t *ddt, size_t nentries, dmu_tx_t *tx, ddt_log_update_t *dlu)
{
	 ...
	 */
	size_t reclen = P2ALIGN(
	    sizeof (ddt_log_record_t) + sizeof (ddt_log_record_entry_t) +
	    DDT_PHYS_SIZE(ddt), sizeof (uint64_t));
	ASSERT3U(reclen, <=, UINT16_MAX);
	dlu->dlu_reclen = reclen;

	uint64_t nblocks = howmany(nentries, DDT_LOG_BSIZE / dlu->dlu_reclen);
	uint64_t offset = ddt->ddt_log_active->ddl_nblocks * DDT_LOG_BSIZE;
	uint64_t length = nblocks * DDT_LOG_BSIZE;
...
ddt_log_update_entry(ddt_t *ddt, ddt_log_t *ddl, ddt_lightweight_entry_t *ddlwe)
{
	...
	for (int p = 0; p < ddlwe->ddlwe_nphys; p++)
		ddle->ddle_phys[p] = ddlwe->ddlwe_phys[p];
...
ddt_log_entry(ddt_t *ddt, ddt_lightweight_entry_t *ddlwe, ddt_log_update_t *dlu)
{
	 ...
	 */
	if (DDT_LOG_BSIZE < (dlu->dlu_offset + dlu->dlu_reclen)) {
	...
	if (dlu->dlu_offset == 0) {
		...
		memset(db->db_data, 0, DDT_LOG_BSIZE);
	}
	...
	for (int p = 0; p < ddlwe->ddlwe_nphys; p++)
		dlre->dlre_phys[p] = ddlwe->ddlwe_phys[p];
...
ddt_log_commit(ddt_t *ddt, ddt_log_update_t *dlu)
{
	...
	ddt->ddt_log_active->ddl_nblocks += dlu->dlu_ndbp;
...
ddt_log_checkpoint(ddt_t *ddt, ddt_lightweight_entry_t *ddlwe, dmu_tx_t *tx)
{
	...
	while (((ddle = avl_first(&ddl->ddl_tree)) != NULL) &&
	    ddt_key_compare(&ddle->ddle_key, &ddlwe->ddlwe_key) <= 0)
		count++;
	VERIFY0(count);
#endif
...
ddt_log_swap(ddt_t *ddt, dmu_tx_t *tx)
{
	 ...
	 * on disk.
	 */
	if (ddt->ddt_log_flushing->ddl_nblocks > 0)
...
ddt_log_load_entry(ddt_t *ddt, ddt_log_t *ddl, ddt_log_record_t *dlr)
{
	...
	for (int p = 0; p < ddlwe.ddlwe_nphys; p++)
		ddlwe.ddlwe_phys[p] = dlre->dlre_phys[p];
...
ddt_log_load_one(ddt_t *ddt, uint_t n)
{
	...
	dmu_buf_t *db;
	err = dmu_bonus_hold(ddt->ddt_os, obj, FTAG, &db);
	if (err != 0)
		return (err);
	memcpy(&hdr, db->db_data, sizeof (ddt_log_header_t));
	dmu_buf_rele(db, FTAG);

	if (hdr.dlh_nblocks > 0) {
		uint64_t length = hdr.dlh_nblocks * DDT_LOG_BSIZE;

		dmu_prefetch(ddt->ddt_os, obj, 0, 0, length,
		    ZIO_PRIORITY_SYNC_READ);

		for (uint64_t offset = 0; offset < length;
		    offset += DDT_LOG_BSIZE) {
			dmu_buf_t *db;
			err = dmu_buf_hold(ddt->ddt_os, obj, offset,
			    FTAG, &db, DMU_READ_PREFETCH);
			if (err != 0)
				/* XXX clear out the log tree */
				return (err);

			uint64_t offset = 0;
			while (offset < DDT_LOG_BSIZE) {
				ddt_log_record_t *dlr =
				    (ddt_log_record_t *)(db->db_data + offset);
	...
	ddl->ddl_nblocks = hdr.dlh_nblocks;
...
ddt_log_load(ddt_t *ddt)
{
	...
	/*
	 * XXX we only need this lock at all to beat the assertion in
	 *     dva_get_dsize_sync(), via ddt_stat_generate(). that seems crazy?
	 *     seems to be working but I should at least confirm that its
	 *     sensible somewhere else in the code -- robn, 2023-06-15
	 */
...
static void
ddt_log_free_one(ddt_t *ddt, ddt_log_t *ddl)
{
	void *cookie = NULL;
	ddt_log_entry_t *ddle;
	while ((ddle =
	...
	ASSERT(avl_is_empty(&ddl->ddl_tree));
	avl_destroy(&ddl->ddl_tree);
}

void
ddt_log_free(ddt_t *ddt)
{
	ddt_log_free_one(ddt, &ddt->ddt_log[0]);
	ddt_log_free_one(ddt, &ddt->ddt_log[1]);
}

...
/*
 * Return the current time expressed in 1/IOLIMIT_RESOLUTION-second slots.
 * Uses the monotonic clock (gethrtime) rather than wallclock - admin time
 * adjustments must not stall or burst rate-limited I/O.
 */
static __inline hrtime_t
gettimeslot(void)
{
	return (gethrtime() / (NANOSEC / IOLIMIT_RESOLUTION));
}
...
{
	ASSERT3P(os->os_dsl_dataset, !=, NULL);

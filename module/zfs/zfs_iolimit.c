...
/*
 * Account for our request across all the types configured in this iolimit
 * structure.
 * Return a timeslot we should wait for or now if we can execute the request
 * without waiting (we are within limits).
 */
static hrtime_t
iolimit_account(struct zfs_iolimit *iol, hrtime_t now, const uint64_t *counts)
{
	hrtime_t timeslot;
	int type;
...
static hrtime_t
iolimit_account_all(objset_t *os, const uint64_t *counts)
{
	dsl_dir_t *dd;
	...
	for (dd = iolimit_first(os, types); dd != NULL;
		...
		ts = iolimit_account(dd->dd_iolimit, now, counts);
		if (ts > timeslot) {
			timeslot = ts;
		}
	}
...
zfs_iolimit_sleep(objset_t *os, const uint64_t *counts)
{
	...
	timeslot = iolimit_account_all(os, counts);

	rrm_exit(&os->os_spa->spa_iolimit_lock, FTAG);

	return (iolimit_sleep(timeslot));
}
    ...
    size_t bytes)
{
	...
	srcts = iolimit_account_all(srcos, counts);
	...
	dstts = iolimit_account_all(dstos, counts);

...
{
	hrtime_t daydiff, monthdiff, diffminute;

	diffminute = time - rrd_tail(&db->dbr_minutes);
	daydiff = time - rrd_tail(&db->dbr_days);
	monthdiff = time - rrd_tail(&db->dbr_months);

	if (diffminute >= 0 && diffminute >= SEC2NSEC(60 * 60))
		rrd_add(&db->dbr_minutes, time, txg);
	if (daydiff >= 0 && daydiff >= SEC2NSEC(24 * 60 * 60))

    ...
    const char *key, dmu_tx_t *tx)
{
	...
	/*
	 * Only promote directory ZAP objects to TinyZAP.  MOS-level ZAPs
	 * (object directory, snapshot maps, props, etc.) and other ZAP
	 * types must remain as plain MicroZAP or FatZAP to preserve
	 * compatibility with bootloaders and older software. This can be
	 * revisited in the future if we want to promote other ZAP types.
	 */
	if (zap->zap_dnode->dn_type != DMU_OT_DIRECTORY_CONTENTS)
		return (B_FALSE);

...
#define	DDT_VERSION_DEFAULT		(DDT_VERSION_FDT)
 ...
 *   1. 'dedup_table_quota' is 0 and ddt_get_ddt_dsize exceeds this limit
 *
 *   2. 'dedup_table_quota' is set to automatic and
 *      a. the dedup allocation class could not satisfy a DDT allocation
 *         in a recent transaction
 *      b. the dedup allocation class has exceeded its calculated limit
 */
static boolean_t
ddt_over_quota(spa_t *spa)
{
	...
	/*
	 * For automatic quota, table size is limited by the size of dedup class
	 */
	metaslab_class_t *dedup_mc = spa_dedup_class(spa);
	if (dedup_mc != NULL && metaslab_class_get_space(dedup_mc) > 0) {
		/* Over quota if allocating outside of dedup allocation class */
		if (spa_syncing_txg(spa) <= spa->spa_dedup_class_full_txg +
		    dedup_class_wait_txgs) {
			/* Waiting for deferred frees to be processed */
			return (B_TRUE);
		}

		/* Use larger of 85% of space or space - 8G */
		uint64_t allocated = metaslab_class_get_alloc(dedup_mc);
		uint64_t capacity = metaslab_class_get_space(dedup_mc);
		uint64_t limit = MAX(capacity * 85 / 100, capacity - (1LL<<33));
		if (allocated >= limit)
			return (B_TRUE);
	}
...
static int ddt_configure(ddt_t *ddt);
...
/*
 * Determine, flags and on-disk layout from what's already stored, or
 * if there is nothing stored, select based on pool config.
 */
static int
ddt_configure(ddt_t *ddt)
{
	spa_t *spa = ddt->ddt_spa;
	char name[DDT_NAMELEN];
	int error;
		...
		if (error == 0) {
			...
			if (ddt->ddt_version != DDT_VERSION_FDT) {
				zfs_dbgmsg("ddt_configure: spa=%s ddt_dir=%s "
				    "unknown version %llu", spa->spa_name,
...
ddt_sync_scan_entry(ddt_t *ddt, ddt_entry_t *dde, dmu_tx_t *tx)
{
	...
	if (refcnt == 0 || nclass < dde->dde_class) {
		ddt_lightweight_entry_t ddlwe;
		DDT_ENTRY_TO_LIGHTWEIGHT(ddt, dde, &ddlwe);
		dsl_scan_ddt_entry(dp->dp_scan, ddt->ddt_checksum, &ddlwe, tx);
	}
}
    ...
    ddt_type_t otype, ddt_class_t oclass, dmu_tx_t *tx)
{
	...
	for (int p = 0; p < DDT_NPHYS(ddt); p++) {
		...
		if (DDT_PHYS_IS_DITTO(ddt, p)) {
			 ...
			 */
			ddt_phys_free(ddt, ddk, ddp, tx->tx_txg);
			continue;
		}
		if (ddp->ddp_refcnt == 0)
...
ddt_sync_table_flush(ddt_t *ddt, dmu_tx_t *tx)
{
	...
	memcpy(&ddt->ddt_histogram_cache, ddt->ddt_histogram,
	    sizeof (ddt->ddt_histogram));
	ddt->ddt_spa->spa_dedup_dspace = ~0ULL;
	ddt->ddt_spa->spa_ddt_dsize = ~0ULL;
	ddt_sync_update_stats(ddt, tx);
}

...
#define	RT_NAME(rt)	(((rt)->rt_name != NULL) ? (rt)->rt_name : "")
    ...
    uint64_t gap)
{
	return (zfs_range_tree_create_impl(ops, type, arg, start, shift, gap,
	    ZFS_RANGE_TREE_F_UC_GENERIC, NULL));
}
...
zfs_range_tree_destroy(zfs_range_tree_t *rt)
{
	...
	if (rt->rt_name != NULL && (rt->rt_flags & ZFS_RANGE_TREE_F_DYN_NAME))
    ...
    boolean_t do_fill)
{
	...
	if (rs == NULL) {
		zfs_panic_recover_rt("zfs: rt_instance=%s: removing "
		    ...
		    "(offset=%llx size=%llx)",
		    RT_NAME(rt), (longlong_t)start, (longlong_t)size);
		return;
	}
	...
	if (!(rstart <= start && rend >= end)) {
		zfs_panic_recover_rt("zfs: rt_instance=%s: removing segment "
		    ...
		    (longlong_t)start, (longlong_t)size,
		    (longlong_t)rstart, (longlong_t)(rend - rstart));
		if (!(rt->rt_flags & ZFS_RANGE_TREE_F_UC_FREE_SPACE))
			return;

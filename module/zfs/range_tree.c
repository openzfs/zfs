...
/*
 * zfs_recover_rt can be set to nonzero to attempt to recover from
 * otherwise-fatal errors while adding or removing unexpected segments to a
 * tree.  When set, calls to zfs_panic_recover_rt() will turn into warning
 * messages. This should only be used as a last resort, as it typically results
 * in leaked space.
 */
static int zfs_recover_rt = B_FALSE;

#define	RT_INSTANCE(rt)	(((rt)->rt_instance != NULL) ? (rt)->rt_instance : "")

static void
zfs_panic_recover_rt(const char *fmt, ...)
{
	va_list adx;

	va_start(adx, fmt);
	vcmn_err((zfs_recover || zfs_recover_rt) ? CE_WARN : CE_PANIC,
	    fmt, adx);
	va_end(adx);
}
...
zfs_range_tree_create_impl(const zfs_range_tree_ops_t *ops,
    ...
    uint64_t gap, uint64_t usecase, const char *instance)
{
    ...
    uint64_t gap)
{
	return (zfs_range_tree_create_impl(ops, type, arg, start, shift, gap,
	    ZFS_RANGE_TREE_UC_UNKNOWN, NULL));
}
...
zfs_range_tree_destroy(zfs_range_tree_t *rt)
{
	...
	if (rt->rt_instance != NULL && (rt->rt_usecase &
	    ZFS_RANGE_TREE_UC_DYN_INSTANCE))
		kmem_strfree((char *)(uintptr_t)rt->rt_instance);
    ...
    int64_t delta)
{
	if (delta < 0 && delta * -1 >= zfs_rs_get_fill(rs, rt)) {
		zfs_panic_recover_rt("zfs: rt_instance=%s: attempting to "
		    "decrease fill to or below 0; probable double remove in "
		    "segment [%llx:%llx]",
		    RT_INSTANCE(rt),
	...
	if (zfs_rs_get_fill(rs, rt) + delta > zfs_rs_get_end(rs, rt) -
	    zfs_rs_get_start(rs, rt)) {
		zfs_panic_recover_rt("zfs: rt_instance=%s: attempting to "
		    "increase fill beyond max; probable double add in "
		    "segment [%llx:%llx]",
		    RT_INSTANCE(rt),
		    (longlong_t)zfs_rs_get_start(rs, rt),
		    (longlong_t)zfs_rs_get_end(rs, rt));
...
zfs_range_tree_add_impl(void *arg, uint64_t start, uint64_t size, uint64_t fill)
{
	...
	if (rs != NULL) {
		...
		if (gap == 0) {
			zfs_panic_recover_rt("zfs: rt_instance=%s: "
			    "adding segment (offset=%llx size=%llx) "
			...
			if (rstart > start)
				zfs_range_tree_add_impl(rt, start,
				    rstart - start, rstart - start);
			if (rend < end)
				zfs_range_tree_add_impl(rt, rend, end - rend,
				    end - rend);
			return;
		}
    ...
    boolean_t do_fill)
{
	...
	if (rs == NULL) {
		zfs_panic_recover_rt("zfs: rt_instance=%s: removing "
		    "nonexistent segment from range tree "
		    "(offset=%llx size=%llx)",
		    RT_INSTANCE(rt), (longlong_t)start, (longlong_t)size);
		return;
	}
	...
	if (rt->rt_gap != 0) {
		if (do_fill) {
			...
			zfs_panic_recover_rt("zfs: rt_instance=%s: freeing "
			    "partial segment of gap tree "
			    "(offset=%llx size=%llx) of "
			    "(offset=%llx size=%llx)",
			    RT_INSTANCE(rt),
			    (longlong_t)start, (longlong_t)size,
			    (longlong_t)rstart, (longlong_t)(rend - rstart));
	...
	if (!(rstart <= start && rend >= end)) {
		zfs_panic_recover_rt("zfs: rt_instance=%s: removing segment "
		...
		if (rstart > start)
			zfs_range_tree_remove_impl(rt, start, rstart - start,
			    do_fill);
		uint64_t mstart = MAX(rstart, start);
		uint64_t mend = MIN(rend, end);
		zfs_range_tree_remove_impl(rt, mstart, mend - mstart, do_fill);
		if (rend < end)
			zfs_range_tree_remove_impl(rt, rend, end - rend,
			    do_fill);
		return;
	}
...
ZFS_MODULE_PARAM(zfs, zfs_, recover_rt, INT, ZMOD_RW,
	"Set to attempt to recover from fatal errors upon modification of "
	"a range tree");

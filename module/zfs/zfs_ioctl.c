...
#if defined(CONFIG_USER_NS) && defined(HAVE_KUIDGID_T)
static int
...
#if defined(CONFIG_USER_NS) && defined(HAVE_KUIDGID_T)
/*
 * On success this consumes the nvlist in pnvp and replaces the pointer with
 * one where ids have been mapped into the current user namespace.
 * In case of error, pnvpn and its pointee stay unmodified.
 */
static int

 ...
 * Copyright (c) 2016, ClusterHQ, Inc. All rights reserved.
 */
...
/* ARGSUSED */
static int
zfs_secpolicy_recv_new(zfs_cmd_t *zc, nvlist_t *innvl, nvlist_t *opts,
    cred_t *cr)
{
	return (zfs_secpolicy_recv(zc, innvl, opts, cr));
}
    ...
    cred_t *cr)
{
	...
	/* Assume version 0 if we see "holds" nvlist */
...
top:
	...
	if (error == 0 && strchr(zl->zl_name, '$') == NULL) {
		dmu_objset_rele(os, FTAG);
		if ((error = dmu_objset_hold(zl->zl_name, FTAG, &os)) != 0)
			return (error);
		/* fill in the stats */
...
zfs_ioc_snapshot_list_next_impl(zfs_list_t *zl, boolean_t simple)
{
	 ...
	 * so exit immediately.
	 */
	if (strlcat(zl->zl_name, "@", zl->zl_namelen) >=
...
zfs_ioc_list_next(zfs_cmd_t *zc, boolean_t snapshot)
{
	...
	zl.zl_namelen = sizeof (zc->zc_name);
...
zfs_ioc_set_prop(zfs_cmd_t *zc)
{
	...
	if (error == 0) {
		nvlist_t *props = zprop_conv_zfs_to_strings(nvl);
		error = zfs_set_prop_nvlist(zc->zc_name, source, props, errors,
		    B_FALSE);
		nvlist_free(props);
	}
...
zfs_ioc_create(const char *fsname, nvlist_t *innvl, nvlist_t *outnvl)
{
	...
	if (nvlist_lookup_nvlist(innvl, "props", &props) == 0) {
		int error = 0;
		nvlist_t *args = fnvlist_dup(innvl);

		fnvlist_remove(args, "props");
		props = zprop_conv_zfs_to_strings(props);
		fnvlist_add_nvlist(args, "props", props);

		error = zfs_stable_ioc_zfs_create(fsname, args, outnvl, NULL,
		    0);
...
zfs_ioc_clone(const char *fsname, nvlist_t *innvl, nvlist_t *outnvl)
{
	 ...
	 */
	if (error == 0) {
		nvlist_t *props = zprop_conv_zfs_to_strings(nvprops);
		error = zfs_set_prop_nvlist(fsname, ZPROP_SRC_LOCAL,
		    props, outnvl, B_FALSE);
		nvlist_free(props);
		if (error != 0)
...
zfs_ioc_recv_new(const char *fsname, nvlist_t *innvl, nvlist_t *outnvl)
{
	...
	*tosnap++ = '\0';
	...
	nvprops = zprop_conv_zfs_to_strings(props);

	error = zfs_ioc_recv_impl(tofs, tosnap, origin, nvprops, force,
...
static int
zfs_stable_ioc_zfs_recv(const char *fsname, nvlist_t *innvl,
    nvlist_t *outnvl, nvlist_t *opts, uint64_t version)
{
	return (zfs_ioc_recv_new(fsname, innvl, outnvl));
}
    ...
    nvlist_t *outnvl, nvlist_t *opts, uint64_t version)
{
	...
	if (nvlist_lookup_string(opts, "log_history", (char **)&history) == 0)
...
{	.zvec_name		= "zfs_receive",
	.zvec_func		= zfs_stable_ioc_zfs_recv,
	.zvec_secpolicy		= zfs_secpolicy_recv_new,
	.zvec_namecheck		= DATASET_NAME,
	.zvec_pool_check	= POOL_CHECK_SUSPENDED | POOL_CHECK_READONLY,
	.zvec_smush_outnvlist	= B_TRUE,
	.zvec_allow_log		= B_TRUE,

...
/*
 * Given a zc with only ZPOOL_CONFIG_LOCK_BEHAVIOR set in its ioctl
 * source nvlist, extract and return the zpool_lock_behavior_t value.
 *
 * Return zpool_lock_behavior_t on success.  On failure, return
 * ZPOOL_LOCK_BEHAVIOR_DEFAULT.
 */
static zpool_lock_behavior_t
zfs_get_zpool_lock_behavior_helper(zfs_cmd_t *zc)
{
	int error = 0;
	...
	/*
	 * Our zfs_cmd_t can include an nvlist with optional input arguments.
	 * Right now the only optional arg is zpool_lock_behavior.  Check if
	 * that exists and if it does, set values accordingly.
	 */
	...
	if (error == 0) {
		if (nvlist_lookup_uint64(nvl, ZPOOL_CONFIG_LOCK_BEHAVIOR,
			...
			if (zpool_lock_behavior >= ZPOOL_LOCK_BEHAVIOR_END) {
				nvlist_free(nvl);
				/* Weird zpool_lock_behavior value */
				return (ZPOOL_LOCK_BEHAVIOR_DEFAULT);
			}
		}
		nvlist_free(nvl);
	}
	return ((zpool_lock_behavior_t)zpool_lock_behavior);
}
...
zfs_ioc_pool_configs(zfs_cmd_t *zc)
{
	...
	zpool_lock_behavior = zfs_get_zpool_lock_behavior_helper(zc);

	if ((zpool_lock_behavior == ZPOOL_LOCK_BEHAVIOR_LOCKLESS) &&
	    (zfs_allow_lockless_zpool_status != 1)) {
		/*
		 * Error - you must have zfs_debug_namespace_lock_delay=1 to
		 * enable --lockless.
		 */
		zc->zc_cookie = ENOTSUP;
		return (ENOTSUP);
	}
...
zfs_ioc_pool_get_props(const char *pool, nvlist_t *innvl, nvlist_t *outnvl)
{
	...
	if (nvlist_lookup_uint64(innvl, ZPOOL_CONFIG_LOCK_BEHAVIOR,
	    &zpool_lock_behavior) == 0) {
		if (zpool_lock_behavior >= ZPOOL_LOCK_BEHAVIOR_END) {
			zpool_lock_behavior = ZPOOL_LOCK_BEHAVIOR_DEFAULT;
		}

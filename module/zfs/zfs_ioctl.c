 ...
 * outnvl: empty
 */
static const zfs_ioc_key_t zfs_keys_pool_prefetch_ddt[] = {
	/* no nvl keys */
};

static int
zfs_ioc_pool_prefetch_ddt(const char *poolname, nvlist_t *innvl,
    nvlist_t *outnvl)
{
	int error;
	spa_t *spa;
	...
	zfs_dbgmsg("pool '%s': loaded ddt into ARC in %llu ms", spa->spa_name,
	    (u_longlong_t)((gethrtime() - start_time) / 1000000));
...
zfs_ioctl_init(void)
{
	...
	zfs_ioctl_register("zpool_prefetch_ddt",
	    ZFS_IOC_POOL_PREFETCH_DDT, zfs_ioc_pool_prefetch_ddt,
	    zfs_secpolicy_config, POOL_NAME,
	    POOL_CHECK_SUSPENDED, B_TRUE, B_TRUE,
	    zfs_keys_pool_prefetch_ddt, ARRAY_SIZE(zfs_keys_pool_prefetch_ddt));

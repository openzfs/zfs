    ...
    uint64_t max_size, uint64_t max_search, uint64_t *found_size)
{
	...
	while (rs != NULL && (rs_get_start(rs, rt) - first_found <=
		...
		if (offset + size <= rs_get_end(rs, rt)) {
			*found_size = MIN(rs_get_end(rs, rt) - offset,
			    max_size);
    ...
    uint64_t *found_size)
{
	...
	rs_set_end(&rsearch, rt, *cursor + max_size);
	...
	if (rs == NULL || (rs_get_end(rs, rt) - rs_get_start(rs, rt)) < size) {
		...
		rs_set_end(&rsearch, rt, MIN(max_possible_size, 1ULL << (hbit +
	...
	if ((rs_get_end(rs, rt) - rs_get_start(rs, rt)) >= size) {
		*found_size = MIN(rs_get_end(rs, rt) - rs_get_start(rs, rt),
		    max_size);
		*cursor = rs_get_start(rs, rt) + *found_size;

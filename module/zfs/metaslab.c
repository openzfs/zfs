    ...
    boolean_t success)
{
	...
	/*
	 * Rotate if we expect next allocation to go far over the allocation
	 * quota for this iteration, but allow more allocations if even after
	 * the next one we expect the vdev queue still below the target (the
	 * vdev is faster than others).  In case we still can't saturate the
	 * vdev, always rotate after we allocated the queue target bytes.
	 */
	...
	if (naq >= mg->mg_queue_target)
		goto rotate;
	if (mga->mga_queue_depth + psize + psize / 2 >= mg->mg_queue_target)
		goto rotate;

	/*
	 * When the pool is not too busy, prefer restoring the vdev free space
	 * balance instead of getting maximum speed we don't need, so that we
	 * could have more flexibility during more busy times later.
	 */
	spa_t *spa = mc->mc_spa;
...
static void
metaslab_group_alloc_increment(spa_t *spa, uint64_t vdev,
    int allocator, int flags, uint64_t psize)
{
	...
	metaslab_group_allocator_t *mga = &mg->mg_allocator[allocator];
	atomic_add_64(&mga->mga_queue_depth, psize);
}

void
metaslab_group_alloc_decrement(spa_t *spa, uint64_t vdev,
    int allocator, int flags, uint64_t psize)
{
	if (!(flags & METASLAB_ASYNC_ALLOC))
		return;
	...
	metaslab_group_allocator_t *mga = &mg->mg_allocator[allocator];
	VERIFY3S(atomic_add_64_nv(&mga->mga_queue_depth, -psize), >=, 0);
}

		...
		if (itx->itx_wr_state == WR_COPIED) {
			...
			case EIO: {
				...
				if (error != 0) {
					 ...
					 * loop over a list of itxs at the top
					 * of zil_lwb_write_issue(), which
					 * itself is called from a loop over a
					 * list of lwbs in various places.
					 * zil_crash() will free those itxs and
					 * sometimes the lwbs, so they are
					 * invalid when zil_crash() returns.
					 * Callers have little choice but to
					 * abort immediately.
					 */
					zil_crash(zilog);
					return (error);
...
zil_crash_clean(zilog_t *zilog, uint64_t synced_txg)
{
	...
	while ((lwb = list_head(&zilog->zl_lwb_crash_list)) != NULL) {
		if (lwb->lwb_alloc_txg >= synced_txg ||
			...
			/*
			 * This lwb was allocated or updated on this txg, or in
			 * the future. We stop processing here, to avoid the
			 * strange situation of freeing a ZIL block on on the
			 * same or earlier txg than what it was allocated for.
			 * We'll get another chance on the next txg.
			 */
			break;
		}
...
zil_commit_writer(zilog_t *zilog, zil_commit_waiter_t *zcw)
{
	...
	/*
	 * If the ZIL failed somewhere inside zil_process_commit_list(), it
	 ...
	 * empty ilwbs, so there will be nothing to issue loop below. That's
	 * why we don't have to plumb the error value back from
...
zil_commit_impl(zilog_t *zilog, uint64_t foid)
{
	 ...
	 * disk, and the pool is suspended so we have no idea when it's coming
	 * back. All we can do is shut down and return error to the caller.
	 */
	ASSERT3U(err, ==, ESHUTDOWN);
	zil_crash(zilog);
	return (err);

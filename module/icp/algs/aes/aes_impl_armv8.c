...
/*
 * These functions are used to execute the ARMv8 Crypto Extensions AES
 * instructions, see asm-aarch64/aes/aesv8-armx.S.
 */
extern ASMABI int aes_v8_set_encrypt_key(const uint32_t cipherKey[],
	int keyBits, uint32_t rk[]);
 ...
 */
static void
aes_armv8_generate(aes_key_t *key, const uint32_t *keyarr32, int keybits)
{
	kfpu_begin();
	(void) aes_v8_set_encrypt_key(keyarr32, keybits,
	    &(key->encr_ks.ks32[0]));
	(void) aes_v8_set_decrypt_key(keyarr32, keybits,
	    &(key->decr_ks.ks32[0]));
	kfpu_end();
}
 ...
 */
static void
aes_armv8_encrypt(const uint32_t rk[], int Nr, const uint32_t pt[4],
    uint32_t ct[4])
{
	kfpu_begin();
 ...
 */
static void
aes_armv8_decrypt(const uint32_t rk[], int Nr, const uint32_t ct[4],
    uint32_t pt[4])
{
	kfpu_begin();
...
static boolean_t
aes_armv8_will_work(void)
{
	return (kfpu_allowed() && zfs_aes_available());
}

const aes_impl_ops_t aes_armv8_impl = {
	.generate = &aes_armv8_generate,
	.encrypt = &aes_armv8_encrypt,
	.decrypt = &aes_armv8_decrypt,
	.is_supported = &aes_armv8_will_work,
	.needs_byteswap = B_FALSE,
	.name = "armv8-ce"
};

#endif /* defined(__aarch64__) && HAVE_SIMD(ARMV8_AES) */

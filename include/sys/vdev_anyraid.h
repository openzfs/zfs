...
typedef struct vdev_anyraid_node {
	avl_node_t	van_node;
	uint8_t		van_id;
	int32_t		van_capacity;
	int32_t		van_next_offset;
} vdev_anyraid_node_t;
...
#define	VDEV_ANYRAID_START_COPES	(VDEV_ANYRAID_MAP_COPIES / 2)
#define	VDEV_ANYRAID_TOTAL_MAP_SIZE(ashift)	(VDEV_ANYRAID_MAP_COPIES * \
	VDEV_ANYRAID_SINGLE_MAP_SIZE(ashift))
#define	VDEV_ANYRAID_START_OFFSET(ashift)	VDEV_ANYRAID_START_COPES * \
	VDEV_ANYRAID_SINGLE_MAP_SIZE(ashift)

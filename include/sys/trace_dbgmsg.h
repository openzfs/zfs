 ...
 *
 * It may be noted that, unlike event classes declared elsewhere, these
 * are defined outside of preprocessor guards. This difference exists
 * because these events use only built-in types so they don't depend on
 * any ZFS headers. Therefore this header is included inside the trace.h
 * multiple inclusion guard and need not be guarded separately. The
 * headers defining other events do have ZFS header dependencies, so
 * they allow multiple inclusion but guard each event class against
 * missing dependencies and multiple declaration.
 */

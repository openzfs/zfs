...
```sh
./contrib/zvol-swap-stall/run.py raw
./contrib/zvol-swap-stall/run.py zvol
```
...
4 GiB of RAM to install build dependencies and build a Git archive of the
current OpenZFS commit. Ignored files and existing build products in the
checkout cannot enter the archive. Later runs reuse that prepared image from
...
The preparation VM sees only the Git archive read-only and writes only to a
temporary status directory. Each test then boots a fresh overlay of the image,
with no network, plus two new file-backed scratch disks. Only a read-only test
configuration and a guest-output subdirectory are shared with the test VM.
The scratch disks use stable virtio serial numbers and host direct I/O. The
payload verifies those serials before modifying either device. The zvol has a
fixed 16 KiB block size. The C pressure helper faults anonymous memory in 8 MiB
steps and records remaining RAM and swap after each step.

The preparation VM needs network access to install Debian packages. Its QEMU
user-mode network is not a security boundary against the source it builds; run
the preparation step only for OpenZFS commits that you trust. Test VMs have no
network.
...
allocation, completed pressure, and successful diagnostics; expiry of the
no-progress timeout during pressure is a stall. The guest lowers the kernel
hung-task timeout to ten seconds, so any blocked-task reports emitted before
QEMU stops are retained in `full.log`. A pass also requires QEMU to shut down
without host intervention after the final guest marker.
The guest kernel must provide `hung_task_timeout_secs` and
`hung_task_warnings` sysctls (`CONFIG_DETECT_HUNG_TASK`) and allow both to be
set to 10. The payload applies and verifies these settings before changing
guest storage, and exits with an error if the diagnostics are unavailable.
...
```sh
./contrib/zvol-swap-stall/run.py zvol --expect pass
```

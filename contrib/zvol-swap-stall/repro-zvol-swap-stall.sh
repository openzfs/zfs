...
# Apply the promised diagnostics before changing guest storage.
for diagnostic in hung_task_timeout_secs hung_task_warnings; do
	diagnostic_path=/proc/sys/kernel/$diagnostic
	[[ -w $diagnostic_path ]] || \
		die "required hung-task diagnostic is not writable: $diagnostic"
	if ! printf '10\n' >"$diagnostic_path"; then
		die "could not set required hung-task diagnostic: $diagnostic"
	fi
	[[ $(<"$diagnostic_path") == 10 ]] || \
		die "required hung-task diagnostic did not retain value: $diagnostic"
done
...
if ! udevadm info --query=property --name "$pool_device" | \
    grep --fixed-strings --line-regexp --quiet 'ID_SERIAL=zvolswap-pool'; then
	die 'unexpected pool device identity'
fi
if [[ $backend == raw ]]; then
	if ! udevadm info --query=property --name "$raw_swap_device" | \
	    grep --fixed-strings --line-regexp --quiet \
	    'ID_SERIAL=zvolswap-raw'; then
		die 'unexpected raw swap device identity'
	fi
fi
...
cleanup()
{
	...
	timeout 60 zpool export "$pool_name"
	return "$status"
}
trap cleanup EXIT

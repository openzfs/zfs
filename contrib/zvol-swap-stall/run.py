...
def create_source_bundle(repository: Path, output: Path) -> None:
    ...
    script_path = "contrib/zvol-swap-stall/prepare-zvol-swap-guest.sh"
    script = output / script_path
    script.parent.mkdir(parents=True)
    script.write_bytes(
        subprocess.check_output(
            ["git", "-C", repository, "show", f"HEAD:{script_path}"]
        )
    )
    script.chmod(0o755)
...
def read_prepare_status(status_file: BinaryIO) -> str:
    status_file.seek(0)
    status = status_file.read(len(b"failure\n") + 1)
    if status == b"success\n":
        return "success"
    if status == b"failure\n":
        return "failure"
    return f"invalid {status!r}"
...
def prepare_guest(
    ...
    try:
        with tempfile.TemporaryDirectory(
            ...
            source = run / "source"
            source.mkdir()
            create_source_bundle(repository, source)
            status = run / "status"
            status.mkdir()
            status_file = status / "prepare.status"
            status_file.touch(mode=0o600)
            seed = cloud_seed(
                cloud_localds,
                run,
            ...
            add_share(command, "source", source, read_only=True)
            ...
            with status_file.open("rb") as status_stream, log_path.open(
                "wb"
            ) as log:
                ...
                try:
                    ...
                    stop(process)
                    raise
                guest_status = read_prepare_status(status_stream)
...
def watch_progress(
    ...
    pressure_done_marker = result_dir / "pressure.done"
    complete_marker = result_dir / "guest.complete"
    last_size: int | None = None
    last_progress = time.monotonic()
    boot_deadline = last_progress + boot_seconds
    started_seen = False
    pressure_done_seen = False
    ...
    while process.poll() is None:
        ...
        if complete_marker.exists():
            return Outcome.PASS
        pressure_done = pressure_done_marker.exists()
        if pressure_done and not pressure_done_seen:
            pressure_done_seen = True
            last_progress = time.monotonic()
        ...
        if pressure_done and now - last_progress >= stall_seconds:
            stop(process)
            return Outcome.ERROR
        if (
            started
            and not pressure_done
    ...
    return Outcome.PASS if complete_marker.exists() else Outcome.ERROR


def last_phase(result_dir: Path) -> Phase:
    if (result_dir / "guest.complete").exists():
...
def run_test(
    ...
    with tempfile.TemporaryDirectory(prefix="zvol-swap-qemu.") as run_name:
        ...
        add_share(command, "zvolresults", guest_dir, read_only=False)
        ...
        if outcome is Outcome.PASS and (
            shutdown_timed_out or qemu_status != 0
        ):
            outcome = Outcome.ERROR

        (result_dir / "outcome").write_text(f"{outcome.value}\n")
        phase = last_phase(guest_dir)

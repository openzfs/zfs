...
DEFAULT_COREDIR=/var/tmp/zloop
DEFAULT_RUNDIR=zloop-run
DEFAULT_VDEVSIZE="512m"
DEFAULT_TIMEOUT=0
DEFAULT_COREMAX=0
DEFAULT_SYMLINK=0
...
function usage
{
	echo -e "\n$0 [-t <timeout>] [ -s <vdev size> ] [-c <dump directory>]" \
	    ...
	    "  output directory ($DEFAULT_COREDIR by default). Any options\n" \
...
#
# On legacy Linux systems the core_pattern will default to "core" and
# cores will be written to the working directory.  Modern systems use
# coredumpctl(1) to manage generated cores.  Prefer whatever method is
# the system default.
#
...
if [[ $coreglob = "*" ]]; then
	echo "Defaulting to coredumpctl(1)"
	ztcoredumpctl=1
fi

function core_file
{
	if [[ $ztcoredumpctl -ne 0 ]]; then
		printf "core"
	else
...
function store_core
{
	...
	# dump the last ztest core for analysis using coredumpctl(1)
	if [[ $ztrc -ne 0 ]] && [[ $ztcoredumpctl -ne 0 ]]; then
		ztpid=$(coredumpctl list -r | awk '/ztest/ {print $5; exit 0}')
		or_die coredumpctl -o "$core" dump "$ztpid" >/dev/null
	fi
...
coredir=${ZLOOP_COREDIR:-$DEFAULT_COREDIR}
basedir=${ZLOOP_WORKDIR:-$DEFAULT_WORKDIR}
rundir=${ZLOOP_RUNDIR:-$DEFAULT_RUNDIR}
timeout=${ZLOOP_TIMEOUT:-$DEFAULT_TIMEOUT}
size=${ZLOOP_VDEVSIZE:-$DEFAULT_VDEVSIZE}
coremax=${ZLOOP_COREMAX:-$DEFAULT_COREMAX}
symlink=${ZLOOP_SYMLINK:-$DEFAULT_SYMLINK}
...
if [[ $timeout -ne 0 ]]; then
	echo "Executing $PROG for $timeout seconds."
fi

...
typedef enum {
	...
	finish_started, /* finish started, need only more output next deflate */
	...
	if (s->status == INIT_STATE) {
		...
		if (s->wrap == 2) {
			...
			if (s->gzhead == NULL) {
				...
				put_byte(s,
				    (Byte)((s->gzhead->time >> 8) & 0xff));
				put_byte(s,
				    (Byte)((s->gzhead->time >> 16) & 0xff));
				put_byte(s,
				    (Byte)((s->gzhead->time >> 24) & 0xff));
				put_byte(s, s->level == 9 ? 2 :

 ...
 * reduced in terms of compatibility with older software. The following
 * is from the original crypt.c. Code woven in by Terry Thorsen 1/2003.
 */
/*
 ...
 * If iCaseSensitivity = 0, case sensitivity is defaut of your
 * operating system
    ...
    voidpf filestream)
{
	...
	if (ZSEEK(*pzlib_filefunc_def, filestream, 0,
	    ZLIB_FILEFUNC_SEEK_END) != 0)
		return (0);
	...
	while (uBackRead < uMaxBack) {
		...
		if (ZREAD(*pzlib_filefunc_def, filestream, buf,
		    uReadSize) != uReadSize)
			break;
 ...
 * If there is files inside the .Zip opened with
 * unzipOpenCurrentFile (see later),
 * these files MUST be closed with unzipCloseCurrentFile before
 * call unzipClose.
    ...
    uLong commentBufferSize)
{
	...
	if (unzlocal_getLong(&s->z_filefunc, s->filestream,
	    &file_info.dosDate) != UNZ_OK)
		err = UNZ_ERRNO;

	unzlocal_DosDateToTmuDate(file_info.dosDate, &file_info.tmu_date);

	if (unzlocal_getLong(&s->z_filefunc, s->filestream,
	    &file_info.crc) != UNZ_OK)
		err = UNZ_ERRNO;
	...
	if ((err == UNZ_OK) && (szFileName != NULL)) {
		...
		if ((file_info.size_filename > 0) && (fileNameBufferSize > 0))
			if (ZREAD(s->z_filefunc, s->filestream,
			    szFileName, uSizeRead) != uSizeRead)
				err = UNZ_ERRNO;
		lSeek -= uSizeRead;
	}
	...
	if ((err == UNZ_OK) && (extraField != NULL)) {
		...
		if (lSeek != 0)
			if (ZSEEK(s->z_filefunc, s->filestream,
			    lSeek, ZLIB_FILEFUNC_SEEK_CUR) == 0)
				lSeek = 0;
			else
				err = UNZ_ERRNO;
		if ((file_info.size_file_extra > 0) &&
			...
			if (ZREAD(s->z_filefunc, s->filestream,
			    extraField, uSizeRead) != uSizeRead)
				err = UNZ_ERRNO;
    ...
    uLong commentBufferSize)
{
	return (unzlocal_GetCurrentFileInfoInternal(file,
	    pfile_info, NULL, szFileName, fileNameBufferSize,
	...
	while (pfile_in_zip_read_info->stream.avail_out > 0) {
		if ((pfile_in_zip_read_info->stream.avail_in == 0) &&
			...
			if (pfile_in_zip_read_info->rest_read_compressed <
			    uReadThis)
				uReadThis = (uInt)pfile_in_zip_read_info->
 ...
 *
 * if buf==NULL, it return the size of the local extra field that
 * can be read if buf!=NULL, len is the size of the buffer,
 * the extra header is copied in buf.
    ...
    uLong uSizeBuf)
{
	...
	if (uReadThis > 0) {
		*szComment = '\0';
		if (ZREAD(s->z_filefunc, s->filestream, szComment,
		    uReadThis) != uReadThis)
			return (UNZ_ERRNO);
	}

    ...
    int error, const char *fmt, ...)
{
	...
	case EDOM:
		...
		zfs_verror(json, hdl, EZFS_BADPROP, fmt, ap);
		break;
    ...
    const char *source, const char *recvd_value)
{
	...
	(void) printf("\n");
		...
		if (json->ld_json) {
			...
			json->nb_buff++;
			json->json_buff = realloc(json->json_buff,
			    sizeof (nvlist_t *) * json->nb_buff);
			((nvlist_t **)json->json_buff)[json->nb_buff - 1] =
			    json->nv_dict_buff;
		}
	}
}
...
zprop_parse_value(libzfs_handle_t *hdl, nvpair_t *elem, int prop,
    ...
    const char *errbuf, zfs_json_t *json)
{
	...
	case PROP_TYPE_NUMBER:
		if (datatype == DATA_TYPE_STRING) {
			...
			if (strcmp(value, "none") == 0) {
				isnone = B_TRUE;
			} else if (zfs_nicestrtonum(json, hdl, value, ivalp)
			    != 0) {
				goto error;
			}

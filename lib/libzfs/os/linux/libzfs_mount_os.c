...
static const char *kMountOptionsProp = "org.linux:mount_options";

int
zfs_os_set_mount_options(libzfs_handle_t *hdl, nvlist_t *ret, nvpair_t *elem)
{
	char *strval;
	...
	if (strcmp(strval, "none") == 0)
		strval = NULL;
	(void) nvlist_remove(ret, nvpair_name(elem), DATA_TYPE_STRING);
	if (strval != NULL)
		return (nvlist_add_string(ret, kMountOptionsProp, strval));
	return (0);
}

int
zfs_os_get_mount_options(zfs_handle_t *zhp, char *propbuf, size_t proplen,
    zprop_source_t *src)
{
	nvlist_t *user_props;
	user_props = zfs_get_user_props(zhp);
	char *mount_options;
	nvlist_t *opts_nvlist = NULL;
	
	if (user_props == NULL)
		return (ENOENT);
	if (nvlist_exists(user_props, kMountOptionsProp) == B_FALSE)
		return (ENOENT);
	opts_nvlist = fnvlist_lookup_nvlist(user_props, kMountOptionsProp);
	mount_options = fnvlist_lookup_string(opts_nvlist, "value");
	if (mount_options && mount_options[0] != '\0' &&
	    strcmp(mount_options, "none") != 0) {
		strlcpy(propbuf, mount_options, proplen);
	} else
		*propbuf = '\0';
	return (0);
}

...
static int
dump_filesystem(zfs_handle_t *zhp, void *arg, zfs_json_t *json)
{
	int rv = 0;
	send_dump_data_t *sdd = arg;
	...
	rv = zfs_iter_snapshots_sorted(zhp, dump_snapshot, arg, json);
	if (!sdd->seenfrom) {
		(void) fprintf(stderr, dgettext(TEXT_DOMAIN,
...
static int
dump_filesystems(zfs_handle_t *rzhp, void *arg, zfs_json_t *json)
{
	send_dump_data_t *sdd = arg;
	nvpair_t *fspair;
	...
	if (!sdd->replicate)
		return (dump_filesystem(rzhp, sdd, json));
...
again:
	...
	for (fspair = nvlist_next_nvpair(sdd->fss, NULL); fspair;
		...
		if (zhp == NULL)
			return (-1);
		err = dump_filesystem(zhp, sdd, json);
    ...
    void *cb_arg, nvlist_t **debugnvp)
{
	...
	if (fromsnap && fromsnap[0] == '\0') {
		...
		return (zfs_error(json, zhp->zfs_hdl, EZFS_NOENT, errbuf));
	}
	...
	if (flags->dedup && !flags->dryrun) {
		...
		if ((err = socketpair(AF_UNIX, SOCK_STREAM, 0, pipefd))) {
			zfs_error_aux(json, zhp->zfs_hdl, strerror(errno));
		...
		if ((err = pthread_create(&tid, NULL, cksummer, &dda))) {
			...
			(void) close(pipefd[1]);
			zfs_error_aux(json, zhp->zfs_hdl, strerror(errno));
			return (zfs_error(json, zhp->zfs_hdl,
	...
	if (flags->replicate || flags->doall || flags->props) {
		...
		if (flags->replicate || flags->props) {
			...
			err = gather_nvlist(zhp->zfs_hdl, zhp->zfs_name,
			    fromsnap, tosnap,
			    flags->replicate, &fss, &fsavl, json);
			if (err)
				goto err_out;
	...
	if (flags->verbose || sdd.snapholds != NULL) {
		 ...
		 */
		sdd.dryrun = B_TRUE;
		err = dump_filesystems(zhp, &sdd, json);
	...
	if (!flags->dryrun && (flags->replicate || flags->doall ||
		...
		if (write(outfd, &drr, sizeof (drr)) == -1) {
			return (zfs_standard_error(json, zhp->zfs_hdl,
			    errno, errbuf));
		}
	}
...
stderr_out:
	err = zfs_standard_error(json, zhp->zfs_hdl, err, errbuf);
err_out:
	fsavl_destroy(fsavl);
	nvlist_free(fss);
	fnvlist_free(sdd.snapholds);
...
zfs_send_one(zfs_handle_t *zhp, const char *from, int fd,
    enum lzc_send_flags flags, zfs_json_t *json)
{
	int err;
	libzfs_handle_t *hdl = zhp->zfs_hdl;
	char errbuf[1024];
	...
	if (err != 0) {
		...
		case EBUSY:
			...
			return (zfs_error(json, hdl, EZFS_BUSY, errbuf));
    ...
    boolean_t byteswap, zio_cksum_t *zc)
{
	...
	free(buf);
	if (err != 0) {
		zfs_error_aux(NULL, hdl, dgettext(TEXT_DOMAIN, "invalid "
    ...
    char **top_zfs, int cleanup_fd, uint64_t *action_handlep)
{
	...
	if (drre.drr_type != DRR_END) {
		error = zfs_error(NULL, hdl, EZFS_BADSTREAM, errbuf);
		goto out;
	}
    ...
    uint64_t *action_handlep)
{
	...
	*strchr(zc.zc_name, '@') = '\0';
	...
	if (ioctl_err != 0) {
		...
		case ENODEV:
			...
			*cp = '\0';
			...
			(void) zfs_error(NULL, hdl, EZFS_BADRESTORE, errbuf);
			*cp = '@';
			break;
		case ETXTBSY:
			zfs_error_aux(NULL, hdl, dgettext(TEXT_DOMAIN,
		...
		case EEXIST:
			...
			(void) zfs_error_fmt(NULL, hdl, EZFS_EXISTS,
			    dgettext(TEXT_DOMAIN, "cannot restore to %s"),
			    zc.zc_value);
			*cp = '@';
			break;
		case EINVAL:
			(void) zfs_error(NULL, hdl, EZFS_BADSTREAM, errbuf);
			break;
		case ECKSUM:
			zfs_error_aux(NULL, hdl, dgettext(TEXT_DOMAIN,
		...
		case EDQUOT:
			...
			(void) zfs_error(NULL, hdl, EZFS_NOSPC, errbuf);
			break;
		default:
			(void) zfs_standard_error(NULL,
	...
	if (cp && (ioctl_err == 0 || !newfs)) {
		...
		*cp = '\0';
		h = zfs_open(NULL, hdl, zc.zc_value,
